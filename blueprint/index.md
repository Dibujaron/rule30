...
117 theorems on the board: 112 proved, 4 open, 1 claimed, 0 blocked, 0 abandoned. 0 of 117 without an object.
...
**What this says.** Doubling commutes with the whole truncated iteration, one bit wider.
**Hypotheses.** (seeded statement; not yet checked)
- `(n t s : ℕ)`
**Conclusion.** `(fun r => ((4 * r) ^^^ ((2 * r) ||| r)) % 2 ^ (n + 1))^[t] (2 * s) = 2 * (fun r => ((4 * r) ^^^ ((2 * r) ||| r)) % 2 ^ n)^[t] s`
**Cited by.** nothing yet
...
**Hypotheses.**
- `(s : ℕ)`
**Conclusion.** `4 * (2 * s) ^^^ (2 * (2 * s) ||| 2 * s) = 2 * (4 * s ^^^ (2 * s ||| s))`
**Cited by.** nothing yet
**Status.** proved, size S, under the wall leftDiagonal_onset_le
