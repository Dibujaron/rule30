...
123 theorems on the board: 116 proved, 6 open, 1 claimed, 0 blocked, 0 abandoned. 0 of 123 without an object.
...
**Cited by.** nothing yet
**Status.** claimed, size S, under the wall leftDiagonal_period_le
...
**What this says.** The period doubles once per eventually-white diagonal, and by nothing else.
**Hypotheses.** (seeded statement; not yet checked)
- `(m q N n : ℕ)`
- `(hq : 0 < q)`
- `(w : ℕ → ℕ)`
...
**Cited by.** nothing yet
**Status.** open, size L, under the wall leftDiagonal_period_le
...
**Cited by.** leftDiagonal_onset_le_of_black_ladder, leftDiagonal_onset_le_of_line, leftDiagonal_step_onset_dichotomy, leftDiagonal_step_period_dichotomy
**Status.** proved, size M, under the wall leftDiagonal_period_le
...
**What this says.** Past a white diagonal, the period doubles exactly when the parity is odd.
**Hypotheses.** (seeded statement; not yet checked)
- `(m q N : ℕ)`
- `(hq : 0 < q)`
- `(h0 : PeriodicFrom (leftDiagonal m) q N)`
...
**Cited by.** column_settledConfig_eq, leftDiagonal_onset_le_iff_rowNat_return, leftDiagonal_onset_le_of_black_ladder, leftDiagonal_onset_le_of_line, leftDiagonal_periodicFrom_pow, leftDiagonal_transient_front_law_pow, minimalPeriod_dvd, rightDiagonal_periodicFrom_pow
**Status.** proved, size S
...
**Hypotheses.** none (verified; statement text from Statements.lean, this proof predates the checked-type block)
**Conclusion.** `(∀ k, ∃ p > 0, ∃ N ≤ k, PeriodicFrom (leftDiagonal k) p N) ↔ ∀ k : ℕ, (stepMod (k + 1))^[2 * k] (1 % 2 ^ (k + 1)) = (stepMod (k + 1))^[2 * k + 2 ^ k] (1 % 2 ^ (k + 1))`
**Cited by.** nothing yet
**Status.** proved, size M, under the wall leftDiagonal_onset_le
