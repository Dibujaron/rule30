...
One entry per theorem, grouped by the object the theorem is about. **What this says** is the sentence a person wrote — the proof note's, or the statement's docstring lead for a node still open. The hypotheses and conclusion are the checked signature as Lean printed it, or the seeded statement where no proof has been checked yet; they are shown, not paraphrased. **Cited by** is read from `import` lines, so it is what `lake build` compiled and not what the board planned.

60 theorems on the board: 47 proved, 13 open, 0 claimed, 0 blocked, 0 abandoned. 0 of 60 without an object.

## Configuration

### evolveFrom_eq_of_agree_on_window

**What this says.** The cone lemma for an arbitrary row.
...
**What this says.** `t` steps of rule 30 are left-permutive with radius `t`.
**Hypotheses.** (seeded statement; not yet checked)
- `(t : ℕ)`
...
**What this says.** The rightmost difference moves right at speed exactly one.
**Hypotheses.** (seeded statement; not yet checked)
- `(c d : Config)`
- `(i : ℤ)`
- `(t : ℕ)`
...
**What this says.** One step of rule 30 is left-permutive with radius 1.
**Hypotheses.** none (seeded statement; not yet checked)
...
**What this says.** The exact local law of the left front.
**Hypotheses.** (seeded statement; not yet checked)
- `(c d : Config)`
- `(i : ℤ)`
- `(h2 : c (i - 2) = d (i - 2))`
...
**What this says.** Flipping the left neighbour always flips the output.
**Hypotheses.** (seeded statement; not yet checked)
- `(c d : Config)`
- `(i : ℤ)`
- `(hl : c (i - 1) ≠ d (i - 1))`
...
**What this says.** The sideways inverse.
**Hypotheses.** (seeded statement; not yet checked)
- `(c : Config)`
- `(i : ℤ)`
**Conclusion.** `c (i - 1) = xor (rule30 c i) (c i || c (i + 1))`
**Cited by.** nothing yet
**Status.** open, size S

### window_count_half

**What this says.** After `t` steps, exactly half of all windows grow a black centre cell.
**Hypotheses.** (seeded statement; not yet checked)
- `(t : ℕ)`
...
**What this says.** The row model agrees with the automaton.
**Hypotheses.** (seeded statement; not yet checked)
- `(t : ℕ)`
- `(x : ℤ)`
