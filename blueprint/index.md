...
113 theorems on the board: 107 proved, 6 open, 0 claimed, 0 blocked, 0 abandoned. 0 of 113 without an object.
...
**Cited by.** leftDiagonal_onset_le_iff_rowNat_return, leftDiagonal_onset_le_of_stepMod_preperiod
...
**What this says.** A leading block of length one gives the same conclusion, whatever the background does.
**Hypotheses.** (seeded statement; not yet checked)
- `(c d : Config)`
- `(i : ℤ)`
- `(hl : c (i - 1) = d (i - 1))`
- `(hc : c i = ! d i)`
- `(h1 : c (i + 1) = d (i + 1))`
**Conclusion.** `xor (rule30 c i) (rule30 d i) = ! d (i + 1)`
**Cited by.** nothing yet
**Status.** open, size S, under the wall leftDiagonal_onset_le

### front_survival_of_white

**What this says.** Where the background is white, the front simply advances.
**Hypotheses.** (seeded statement; not yet checked)
- `(c d : Config)`
- `(i : ℤ)`
- `(hl : c (i - 1) = d (i - 1))`
...
**Cited by.** leftDiagonal_onset_le_iff_stepMod_return, leftDiagonal_onset_le_of_stepMod_preperiod
**Status.** proved, size M, under the wall leftDiagonal_onset_le
