...
71 theorems on the board: 60 proved, 11 open, 0 claimed, 0 blocked, 0 abandoned. 0 of 71 without an object.

## Configuration

### config_eq_of_right_and_column

**What this says.** A row is rigid from the right.
...
**Status.** open, size M, under the wall centerColumn_other_isEventuallyPeriodic_of_center
...
**What this says.** The left half-line is exact.
**Hypotheses.** (seeded statement; not yet checked)
- `(X : Config)`
- `(t k : ℕ)`
**Conclusion.** `evolveHalfLeft (column X 0) (fun k => X (-((k : ℤ) + 1))) t k = column X (-((k : ℤ) + 1)) t`
**Cited by.** nothing yet
**Status.** open, size M, under the wall centerColumn_other_isEventuallyPeriodic_of_center

### evolveHalfRight_eq_column

**What this says.** The right half-line is exact.
**Hypotheses.** (seeded statement; not yet checked)
- `(X : Config)`
- `(t k : ℕ)`
**Conclusion.** `evolveHalfRight (column X 0) (fun k => X ((k : ℤ) + 1)) t k = column X ((k : ℤ) + 1) t`
**Cited by.** nothing yet
**Status.** open, size M, under the wall centerColumn_other_isEventuallyPeriodic_of_center

### leftSolve_eq_column

**What this says.** The sideways solve is exact.
**Hypotheses.** (seeded statement; not yet checked)
- `(X : Config)`
- `(k t : ℕ)`
**Conclusion.** `leftSolve (column X 0) (column X 1) k t = column X (-(k : ℤ)) t`
**Cited by.** nothing yet
**Status.** open, size M, under the wall centerColumn_other_isEventuallyPeriodic_of_center
...
**What this says.** The seed's own black-time law.
**Hypotheses.** (seeded statement; not yet checked)
- `(t : ℕ)`
...
**What this says.** At a white centre cell, column `1` is forced
**Hypotheses.** (seeded statement; not yet checked)
- `(X : Config)`
- `(t : ℕ)`
- `(h : column X 0 t = false)`
**Conclusion.** `column X 1 t = xor (column X 0 (t + 1)) (column X (-1) t)`
**Cited by.** nothing yet
**Status.** open, size S, under the wall centerColumn_other_isEventuallyPeriodic_of_center

### column_succ_of_black

**What this says.** At a black centre cell the next centre cell is the complement of the cell to its left
**Hypotheses.** (seeded statement; not yet checked)
- `(X : Config)`
- `(t : ℕ)`
- `(h : column X 0 t = true)`
