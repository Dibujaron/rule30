...
87 theorems on the board: 80 proved, 5 open, 2 claimed, 0 blocked, 0 abandoned. 0 of 87 without an object.
...
**What this says.** An odd driver makes the next right diagonal antiperiodic, and pins its minimal period at `2 * L`.
**Hypotheses.** (seeded statement; not yet checked)
- `(k L : ℕ)`
- `(hL0 : PeriodicFrom (rightDiagonal k) L 0)`
- `(hL1 : PeriodicFrom (rightDiagonal (k + 1)) L 0)`
- `(hodd : Odd ((Finset.range L).sum (fun j => if rightDiagonal (k + 1) (j + 1) || rightDiagonal k (j + 2) then 1 else 0)))`
**Conclusion.** `(∀ j, rightDiagonal (k + 2) (j + L) = ! rightDiagonal (k + 2) j) ∧ PeriodicFrom (rightDiagonal (k + 2)) (2 * L) 0 ∧ ¬ PeriodicFrom (rightDiagonal (k + 2)) L 0`
**Cited by.** nothing yet
**Status.** claimed, size M
...
- `(hanti : ∀ j, rightDiagonal (k + 1) (j + q) = ! rightDiagonal (k + 1) j)`
- `(j : ℕ)`
...
**What this says.** No right diagonal past the edge is constant.
**Hypotheses.** (seeded statement; not yet checked)
- `(k : ℕ)`
- `(hk : 1 ≤ k)`
