...
102 theorems on the board: 97 proved, 5 open, 0 claimed, 0 blocked, 0 abandoned. 0 of 102 without an object.
...
**What this says.** The centre column does not determine the seed.
**Hypotheses.** (seeded statement; not yet checked)
- `(k : ℕ)`
**Conclusion.** `∃ c : Config, c (2 * k : ℤ) = true ∧ (∀ i : ℤ, (2 * k : ℤ) < i → c i = false) ∧ (∀ t : ℕ, 1 ≤ t → column c 0 t = centerColumn t)`
**Cited by.** nothing yet
**Status.** open, size L, under the wall centerColumn_right_isEventuallyPeriodic_of_center
