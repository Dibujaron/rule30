...
## 2026-09-12T17:21:08Z — rightDiagonal_edge_gap_eq (opus, proved)

Retrieval, not derivation. The brief named `explorer/sextant10_scratch_cone.lean` and the theorem `edge_gap_eq` in it is this node's statement character for character. Copied, pruned, built first try, ~$1.1 of work on ~$1.9 of prompt. This is the same scratch file I took `rightDiagonal_cone` out of earlier today — **one scratch file can serve several nodes, and the second visit is cheaper because you already know its shape.** Worth checking, when a brief names a scratch you have seen before, whether your own notebook entry for the earlier node already tells you the import list and the pruning decisions (mine did).

**The pruning, since the two nodes cut the file in different places.** `rightDiagonal_cone` needed `rightDiagonal_periodicFrom_pow_le` → `cone_row` → `cone_row_pow` → `cone_rightDiagonal`. This node needs `cone_row` (shared) plus `edge_black_within`, `antiperiodic_odd_mul`, `edge_gap`, the five `rd_*` minimal-period helpers, and `edge_gap_eq_aux`. So `cone_row` is the only overlap, and because it is `private` in both files it is correctly duplicated rather than imported — same situation as `testBit_rowStep` across the rowNat family. Dropped from this node: `rightDiagonal_periodicFrom_pow_le`, `cone_row_pow`, `cone_rightDiagonal`, `a_ge`, `backward_line_step`, `term`, `lineXor`, and the three `example`/`decide` kernel checks. Kept the scratch's full nine-import list unchanged; every one is still reached (`EvolveLeftEdge` only through `edge_black_within`'s crude 2p witness, which is easy to think is dead weight and is not).

**Trace the dependency chain by hand before deleting, not after.** `rd_minimalPeriod_pow` looks like it is only there for the docstring's "periods are powers of two" remark; it is actually load-bearing, cited by `rd_minimalPeriod_dvd_of_le`, which the top-level `le_total` split needs in both branches. Deleting it would have cost a build cycle.

**The one real idea in the proof, recorded because the statement hides it.** `hhi` is a NEGATIVE hypothesis (a minimal period fails to divide p) and the conclusion needs a POSITIVE fact (diagonal D+2 is the exact complement of itself p steps on). The bridge is two uses of `minimalPeriod_dvd` as contrapositives: if the driver's weight over one period L were even, `rightDiagonal_periodicFrom_step_of_even_driver` would make L a period of level D+2 and its minimal period would divide L hence p — so the weight is odd, and `rightDiagonal_antiperiodic_of_odd_driver` fires. Then the SAME move again on `2 * L` (via `rightDiagonal_periodicFrom_step`) forces the cofactor `p / L` to be odd, which is what lets `antiperiodic_odd_mul` carry antiperiodicity from L up to p. **Two contrapositives of one served lemma, at two different arguments — that is the whole trick, and neither is visible from the statement.**

**`le_total` on the two minimal periods, not a construction of their lcm.** `edge_gap_eq_aux` wants ONE common period L of diagonals D and D+1. Since both minimal periods are powers of two, the larger is a multiple of the smaller, so `rcases le_total ... with h | h` and taking whichever is larger is enough — no `Nat.lcm` and no gcd bookkeeping. Reusable whenever two quantities are known to be powers of the same prime: `rd_minimalPeriod_dvd_of_le` is four lines (`Nat.dvd_prime_pow`, then `Nat.pow_le_pow_iff_right`) and does the job of a lattice argument.

**`set` worked fine with this import surface** (`set m := Nat.find hex with hmdef`, twice). Another data point against my old blanket note that `set` is unavailable from a bare-Basic file — see [[rightDiagonal-driver-flip-iff-white]]. Try it; do not route around it.

**`push_neg` deprecation warnings are cosmetic and I left them.** Two of them, at the two `by_contra; push_neg` openings. `lake build` succeeds, the harness verifies, and rewriting to `push Not` would be an untested change to a proof I did not write. Consistent with what other notebooks in this project record. A future reader will see them in this file's build log — they are mine, deliberately.

Axioms verified by the append-`#print axioms`-then-`lake env lean`-then-remove trick rather than relayed from the scratch's own print: `propext, Classical.choice, Quot.sound`. `Classical.choice` is genuinely load-bearing (two `classical` blocks feeding `Nat.find`'s decidability).

**Band, and this is the part to carry forward rather than the tactics.** The brief's own Band field is unusually careful and it is right: KNOWN-ADJACENT WITH A NEW PROOF, and the "new" is UNESTABLISHED because Sextant did not read Rowland's section 3. Do not upgrade this to novel in any later report, and do not cite this node as evidence the two-way form is unpublished. It bears on no prize conjecture and sits behind the same fence as `rightDiagonal_cone` — the right edge was closed for P1 from two independent directions the same day this landed.
