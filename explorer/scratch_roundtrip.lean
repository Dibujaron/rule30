...
#print axioms RoundTrip.rule30_alternating_step
#print axioms RoundTrip.column_black_run_of_alternating
#print axioms RoundTrip.column_alternating_of_black_run
#print axioms RoundTrip.column_alternating_shrink
#print axioms RoundTrip.centerColumn_not_isEventuallyPeriodic_of_long_black_runs
#print axioms RoundTrip.centerColumn_not_isEventuallyPeriodic_of_deep_alternating

#eval (List.range 7).all (fun t => (List.range 5).all (fun j => ((List.range (j + 1)).all (fun s => evolve (t + s) 0) == (List.range (j + 1)).all (fun i => evolve t (-(i : ℤ)) == decide (i % 2 = 0)))))
#eval (List.range 7).all (fun t => (List.range 5).all (fun j => ((List.range (j + 1)).all (fun s => evolve (t + s) 0) == (List.range (j + 1)).all (fun i => evolve t (-(i : ℤ)) == decide (i % 2 = 0)))))
