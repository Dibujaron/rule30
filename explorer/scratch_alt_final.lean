...
/-
Final check: each of the five proposed statements, written EXACTLY as it will
land, followed by the tactic block that will be recorded as its route.  Each
route is self-contained (no private helper outside the block), so the block a
prover is handed is the block that was checked here.
-/
...
#print axioms Final.rule30_alternating_step
#print axioms Final.column_black_run_of_alternating
#print axioms Final.column_alternating_of_black_run
#print axioms Final.centerColumn_not_isEventuallyPeriodic_of_long_black_runs
#print axioms Final.centerColumn_not_isEventuallyPeriodic_of_deep_alternating

-- The witness for the two run/alternating nodes, as it will be recorded.
#eval (List.range 7).all (fun t => (List.range 5).all (fun j =>
  ((List.range (j + 1)).all (fun s => evolve (t + s) 0)
    == (List.range (j + 1)).all (fun i => evolve t (-(i : ℤ)) == decide (i % 2 = 0)))))
