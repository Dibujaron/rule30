/- generated from blueprint/proposals/next.json; do not edit -/
import Rule30.Basic
import Rule30.Prize
...
#eval (List.range 10).all fun n => decide (centerColumn n = ((stepMod (n + 1))^[n] (1 % 2 ^ (n + 1))).testBit n)
#eval (List.range 6).all fun M => (List.range 5).all fun k => decide (((Finset.range (M + k)).filter fun n => centerColumn n = true).card = ((Finset.range M).filter fun n => centerColumn n = true).card + ((Finset.range k).filter fun j => centerColumn (M + j) = true).card)
#print axioms centerColumn_density_tendsto_half_iff_excess_nat
#print axioms centerColumn_density_tendsto_half_of_nearby_cuts
#print axioms centerColumnCount_eq_stepMod_count
#print axioms centerColumnCount_block
