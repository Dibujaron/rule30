...
- The only file you may edit is `Rule30/Proofs/IsEventuallyPeriodicOfPeriodicStep.lean`. Every other write is denied by a hook, not by convention.
- The only shell commands you may run are `lake build Rule30.Proofs.IsEventuallyPeriodicOfPeriodicStep` and `lake env lean <file>`. Everything else is denied, and so is any shell operator — no `;`, `&&`, `|`, backticks, `$`, `>` or `<`. One bare command per Bash call.
...
- The harness verifies with a generated check theorem — `theorem harness_check : type_of% Statements.isEventuallyPeriodic_of_periodic_step := isEventuallyPeriodic_of_periodic_step` — against the captain's statement. So your theorem must be named exactly `isEventuallyPeriodic_of_periodic_step` and have exactly the stated type. A weakened or generalized restatement fails, even one you could prove.
...
still in progress when the round budget ran out: Proof is complete and lake-build-clean; ending this attempt as in_progress since the harness check file is still unregenerated and no further action is possible from within this file's scope.
