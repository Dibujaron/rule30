...
- The harness verifies with a generated check theorem — `theorem harness_check : type_of% Statements.bool_driven_periodicFrom_of_reset := bool_driven_periodicFrom_of_reset` — against the captain's statement. So your theorem must be named exactly `bool_driven_periodicFrom_of_reset` and have exactly the stated type. A weakened or generalized restatement fails, even one you could prove.
