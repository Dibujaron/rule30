...
- The harness verifies with a generated check theorem — `theorem harness_check : type_of% Statements.centerColumnCount_succ := centerColumnCount_succ` — against the captain's statement. So your theorem must be named exactly `centerColumnCount_succ` and have exactly the stated type. A weakened or generalized restatement fails, even one you could prove.
