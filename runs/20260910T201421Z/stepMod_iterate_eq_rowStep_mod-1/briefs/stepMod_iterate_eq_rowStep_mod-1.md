...
- The harness verifies with a generated check theorem — `theorem harness_check : type_of% Statements.stepMod_iterate_eq_rowStep_mod := stepMod_iterate_eq_rowStep_mod` — against the captain's statement. So your theorem must be named exactly `stepMod_iterate_eq_rowStep_mod` and have exactly the stated type. A weakened or generalized restatement fails, even one you could prove.
