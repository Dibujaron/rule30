...
- The harness verifies with a generated check theorem — `theorem harness_check : type_of% Statements.evolveHalfLeft_eq_column := evolveHalfLeft_eq_column` — against the captain's statement. So your theorem must be named exactly `evolveHalfLeft_eq_column` and have exactly the stated type. A weakened or generalized restatement fails, even one you could prove.
