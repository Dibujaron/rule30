...
- The harness verifies with a generated check theorem — `theorem harness_check : type_of% Statements.column_succ_of_black := column_succ_of_black` — against the captain's statement. So your theorem must be named exactly `column_succ_of_black` and have exactly the stated type. A weakened or generalized restatement fails, even one you could prove.
