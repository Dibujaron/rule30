...
- The only file you may edit is `Rule30/Proofs/NotIsEventuallyPeriodicAdjacent.lean`. Every other write is denied by a hook, not by convention.
- The only shell commands you may run are `lake build Rule30.Proofs.NotIsEventuallyPeriodicAdjacent` and `lake env lean <file>`. Everything else is denied, and so is any shell operator — no `;`, `&&`, `|`, backticks, `$`, `>` or `<`. One bare command per Bash call.
...
- The harness verifies with a generated check theorem — `theorem harness_check : type_of% Statements.not_isEventuallyPeriodic_adjacent := not_isEventuallyPeriodic_adjacent` — against the captain's statement. So your theorem must be named exactly `not_isEventuallyPeriodic_adjacent` and have exactly the stated type. A weakened or generalized restatement fails, even one you could prove.
