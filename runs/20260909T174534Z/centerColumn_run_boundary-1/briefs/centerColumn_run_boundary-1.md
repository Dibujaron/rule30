...
- The harness verifies with a generated check theorem — `theorem harness_check : type_of% Statements.centerColumn_run_boundary := centerColumn_run_boundary` — against the captain's statement. So your theorem must be named exactly `centerColumn_run_boundary` and have exactly the stated type. A weakened or generalized restatement fails, even one you could prove.
