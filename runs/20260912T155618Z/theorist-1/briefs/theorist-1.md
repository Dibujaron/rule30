...
## 2026-09-12T01:03:08Z — The row count b(t): is the row marginal provable where the column marginal is not? THIS IS TALUS'S OWN NEXT TOPIC, section 6 of docs/attacks/2026-09-10-prize-2-s-residual-*; read that document's sections 3 (C3 and C4 especially), 4 and 6 before anything else. THE ONE PIECE OF CONTENT BEHIND IT: rule 30's local law constrains a ROW and not a COLUMN. C3 bounds the triangle's black density by 2/3 in two lines; C4 gives b(t+1) exactly from row t's run structure; and NO ARGUMENT OF EITHER SHAPE EXISTS OR CAN EXIST for the centre column, because crystal 40 makes the centre column free while the row is not free at all. Meanwhile centerColumn_run_boundary (closed, on the board) makes the row count P2's SIBLING MARGINAL exactly: count(N) is the number of rows whose run-boundary set contains the origin, and the size of that set in row t is b(t+1). YOUR TARGET, which is C4's identity turned into a question: since b(t+1) = rho(t) + 2*G2(t) + 2, row balance is exactly the statement that A RULE 30 ROW HAS ABOUT ONE MAXIMAL BLACK RUN PER FOUR CELLS AND ABOUT HALF ITS INTERIOR GAPS ARE LONG. That is a claim about the run structure of a SINGLE ROW -- a finite object -- where P2 is a claim about a limit along a direction the rule does not constrain. Find what is provable about rho(t) and G2(t). The inequality is already tight at t = 1, so the method has no slack being wasted. THE CAVEAT TALUS NEARLY GOT BACKWARDS, and you must not: THE ROW EXCESS IS NOT ANOMALOUS. The quantity |2b(t) - (2t+1)| reaches 3061 at t = 285762, which is 4.05*sqrt(2t+1) -- but that is a maximum over 3*10^5 rows and the maximum of that many draws is about 4.5 sigma, so the row marginal looks as much like a coin as the column does. ATTACK IT FOR ITS PROVABILITY, NOT FOR ANY MEASURED ANOMALY, and if you find yourself reporting an anomaly, compute the null model's maximum first. The row density is A070952; crystal 29 calls it purely empirical and says only the cone is provable; nobody on this board has attacked it. A NEGATIVE IS A RESULT: if rho(t) and G2(t) are as unconstrained as the column, say so and P2's last named route closes. Do not propose a thirteenth P2 lemma quantified over Config -- C2 of Talus's document proves nothing of that shape can bound the excess. (opus, attacked)

What I was wrong about, in order. My ring enumeration ran rule 86: I wrote `l = s >>> 1` and called it the left neighbour, but bit i of `s >>> 1` is s(i+1), so it was `right XOR (centre OR left)` — the mirror. It produced my headline 3/5 correctly, because density is mirror-invariant and 10011 reversed is 11001, the same cyclic word; every symmetric check passed and I would have shipped it. What caught it was asymmetric: rule 30's known period-3 ring 010011111000 (Wolfram Table 6.2, the witness of two earlier obstruction entries) was absent from the N=12 output. Check a mirrored engine against a known ASYMMETRIC object, not against a number you like. The same script totalled cycle weights out of a 4096-entry path buffer, so its N=17 minimum (cycle period 10,846) came back 0.374521 where the truth is 0.492647 — sizing a buffer by "cycles are short" when the enumeration reports the period. I then read that period-3 ring's weights as 7,6,6 = 19/36 and spent an hour believing there was a second witness above 1/2; recount gives 6,6,6 = 18/36, exactly balanced. I expected the block-statistics hierarchy to FAIL to close (level k needing level k+1 statistics) and planned the document around that; it closes at level 1, at the wrong value, which is a better answer and the opposite shape. My first "LP value iteration" iterated the constraint as an EQUALITY, d' = min(3d, 1.5(1-d)), which is a chaotic tent map averaging 0.5 — an inequality-constrained optimum needs a DP, which gives 0.6057 at T=400. And I reproduced Talus's own t=0 phantom-failure artifact (comparison against an uninitialised predecessor) before reading far enough into his section 5 to find it recorded.

What held. 2b(t+1)+3b(t) ≤ 6t+9, 0 failures over 2e5 rows and over EVERY word of span ≤ 22 (2,097,152 words) with min slack exactly 0 at every span, extremal word (100)^k1; kernel-accepted in sextant8_scratch_rows.lean against rowCell with sextant8_scratch_mutant.lean rejected at all three lines. The ring 10011: period 5, every state weight 3, density exactly 3/5 — hand-verified, kernel-verified, and the corrected enumeration over every cycle of every ring N ≤ 24 finds nothing above it (which C1 predicts, since C1 proves ≤ 3/5 for every configuration). Planted in a cone: triangle density 0.599917 at span 4e6 over 2000 rows, so cone-aware arguments do not escape. Lower side: all-white gives 0, best non-trivial 0.456522 at N=23. Row 200,000 has a coin word's block statistics at every k ≤ 16 (chi2 BELOW the coin's throughout). Row excess sub-coin: 20 of 20 null draws at least as extreme; lag-1 autocorrelation −0.4911 is the only rule-30-specific row statistic I found.

Sources searched: all of sources/ for invariant measure, maximal/maximum/highest density, equidistribut, A070952, row density, upper bound + density, maximal black run, number of runs, run boundar, two-thirds; wolfram-1986 for invariant/extremal/maximis/maximiz (10 hits, all about invariant configurations as objects or entropy, none about density). Wolfram Table 6.2 prints periods up to 4 and stops, so the period-5 element is not in print; Talus's 2026-09-08 enumeration has it and its novelty was correctly refused as a table continuation. Fukś 2013 §4 and Wolfram §4 are ensemble densities from disordered starts, not the single seed.

What I would try next: the 4^{-t} reachable set (section 6). Do NOT re-run the k-block refinement, the cone-as-escape, the run-length moments, the row excess, or the ring enumeration.

## 2026-09-12T01:57:01Z — The 4^-t constraint: what does 'row t is a t-step image' actually forbid? THIS IS YOUR OWN NEXT TOPIC, Sextant, section 6 of docs/attacks/2026-09-12-the-row-count-b-t-* which you wrote tonight; read your own sections 3 C5, 4 and 6 first. WHY IT IS THE ONE THING LEFT STANDING, in your words: your session closed the local route with a sharp constant and left exactly this untouched. Rule 30 is pre-injective (crystal 4), so the map from a finite configuration to row t is INJECTIVE, and only 2^(m-1) of the 2^(m+2t-1) words of span m+2t occur as row t -- a 4^-t-sparse set, two bits of information destroyed per step. That is a genuine, quantitative, PROVABLE constraint on a row, and it is the precise sense in which the row is not free where the centre column is. Yet your own C5 measures the seed's row as locally indistinguishable from a coin word at every block size to 16, so the whole of that constraint is invisible to every local statistic -- and your C2 shows that is not an accident, since a local statistic cannot see past an invariant measure. THE QUESTION: is there any computable description of the reachable set at all? CONCRETELY AND FALSIFIABLY, your own formulation: by surjectivity (crystal 3 and 6) every word is an image, so being a 1-step image forbids nothing. Is the same true of being a t-step image OF A FINITE CONFIGURATION, for each fixed t, in the sense that every word of the right span occurs in some WINDOW of such a row? A NEGATIVE -- an explicit word of some length occurring in NO row of ANY finite configuration's picture at depth t -- would be THE FIRST NON-LOCAL CONSTRAINT ON A RULE 30 ROW ANYONE HAS PRODUCED. It is cheap to search exhaustively at small t by enumerating preimages leftward from the white tail; do that search and report the result either way. A POSITIVE, which you said you expect, converts your fence from 'local arguments cannot do it' into 'nothing short of the full orbit can', which is worth stating plainly because it would close the row marginal the way crystal 40 closed the column. Either way it is decidable at small depth, which is more than the row's other questions are. DO NOT send yourself at a k-block refinement of C1, at the cone as an escape from the ring, or at the row excess: all three are closed in your own document, with witnesses. ONE CORRECTION TO CARRY: your 3/5 claim was checked by the captain and holds, and holds harder than you claimed -- exhaustive over every ring size 1 to 18, no rule 30 cycle exceeds density 3/5, and 3/5 is attained at exactly n = 5, 10, 15, that is 10011 and its repeats and nothing else. Output is in explorer/rowan_ring_density_check.txt. Its NOVELTY is unestablished: your evidence was absence from sources/, which is absence in a local corpus, so treat it as a candidate and do not build on it as known. (opus, attacked)

What I was wrong about, in order. I planned the session around finding a word that is no row's factor at depth t, because the topic asked for it and I had written the topic; it dies in the first script, constructively, because surjectivity plus the light cone builds the configuration outright, and I should have seen that before commissioning myself. My first reachability criterion required the preimage's support to begin at the image's leftmost index; it begins one cell in, since the cone shrinks going backwards, and the result was 27,264 false negatives, exactly half at every step. The tell was a survival rate printing 0.000000, impossible for a procedure claimed exact — the number was true of what it measured. I then reported the corrected 0.250000 as a measurement of two fair bits for about an hour; it is 2^(n-2t-2) over 2^(n-2t) restated and would read 0.250000 under any mechanism. My first kernel form of the unreachability check compared the image to 11101 on the five cells of the word rather than to the configuration on [-4,8]; Lean proved it false in one run, correctly, because a configuration black outside a window can match inside it — the exact window/whole-row confusion the document is about, made inside the file that checks it. And I nearly published a next topic ("the sweep state is a new coordinate on the centre column") that would have been a rediscovery; working out what the state actually is — right diagonal t at negative index — took twenty minutes and turned it into something better.

What held. C1: every word is a window of row t of a finite configuration, 688,110 (t,word) pairs at t<=8, 0 failures, plus a brute-force factor sweep over every config of span <=18 (256/256 at length 8), plus kernel k1_window. C2: the white-tail backward solve decides membership exactly — 2,258,262 words against brute force, 0 disagreements either way, count 2^(n-2t-2) exact at every (t,n), finite preimage unique at 2,048 images. C3: reachability is not a prefix+suffix condition at any width at any t<=7; smallest witness 11101 (span 5), hand-verified, kernel-verified over every config supported in [-3,7], with sextant9_scratch_mutant.lean rejected at the right line; at t=8 a flip anywhere kills reachability with prob >=0.99. C4: minimal DFA 3,7,16,35,71,141,272,517,971,...,526563 at t=1..20, ratios 2.33 down to 1.728 — and these are obstruction 11's nucleus forced-set sizes, four of five exact including both six-digit entries, with 517 vs 532 at t=8 unresolved. C5: density L/(L+4t) -> 1, measured 0.971875 at t=40,L=4000, so no density ceiling; the 10011 ring planted as a finite config holds 0.600002 at every t<=60. Unlooked-for cross-check: the all-black span-n word is a 1-step image exactly when 3|n with preimage (100)^k, which is crystal 25's ring result under a different boundary condition.

Sources searched: sources/ for preimage, pre-image, predecessor, inverse, leftward, solve, finite configuration, finite support, Garden, reachable, image of, regular language, sofic, transducer, automaton recognis, quiescent — 95 hits over 7 files, read at Wolfram 1986 §4 (lines 447-456, the four-predecessors leftward solve) and §9 (lines 1251-1268, the ring predecessor structure and K=1.696), Fuks 2013 §1-2, and martinez-adamatzky-hoffmann §5 (the de Bruijn subset-diagram method for Garden-of-Eden words, and Amoroso-Cooper as its reference [29], paper not held). Amoroso-Cooper is attributed from memory and flagged as such in the document.

What I would try next: not this. Section 6's backward run of the right-diagonal recurrence, which contradicts a standing obstruction and is grounded in sextant9_sweepstate.mjs. Do not re-run the window construction, the criterion sweep, the automaton minimisation, the density range, or the planted ring.
...
Topic: Run the right-diagonal recurrence BACKWARDS -- your own next topic, section 6 of docs/attacks/2026-09-12-the-4-t-constraint-*.md, which no session has touched. Your own words there: the backward sweep at input position p carries, at level k, the cell (t-k, p-k); at p = 0 those cells are (t-k, -k), the line time - position = t, which is right diagonal t at index -k. So the sweep state at the origin is the NEGATIVE-INDEX EXTENSION of a right diagonal, and rightDiagonal k j = evolve (j+k) j is well defined down to j = -k, where it reads the initial row at position -k. The task is to write the cone condition that the flat tower fails inside the tower own vocabulary, and to say whether that condition is a statement the board can carry. Why this and not something else: the DAG is exhausted -- 165 of 169 nodes proved, and the four open ones are all walls, all P1 -- so the only thing that can move is new mathematics, and the right edge is where periodicity is exact with no transient, which is the opposite of the left edge trap CLAUDE.md warns about. Aimed at P1 and specifically at the two open fence walls centerColumn_right_isEventuallyPeriodic_of_center and centerColumn_other_isEventuallyPeriodic_of_center: closing either gives Prize 1 outright, since centerColumn_not_eventually_periodic_of_any_other is already proved. Do not re-derive: the group generated by the three-state Mealy automaton E is Z, E is not level-transitive, and the k-block refinement of C1, the cone as an escape from the ring, and the row excess are all closed with witnesses. A Portage connector is live in parallel on the SAME right edge from the self-similar-group side; you are the recurrence side of the same object and need not duplicate it.

Attack document: c:/Users/dibuj/dev/rule30/docs/attacks/2026-09-12-run-the-right-diagonal-recurrence-backwards-your-own-next-topic-section-6-of-docs-attacks-2026-09-12-the-4-t-constraint.md
