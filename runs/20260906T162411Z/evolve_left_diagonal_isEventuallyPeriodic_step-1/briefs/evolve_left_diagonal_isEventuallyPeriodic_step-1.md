...
- The only file you may edit is `Rule30/Proofs/EvolveLeftDiagonalIsEventuallyPeriodicStep.lean`. Every other write is denied by a hook, not by convention.
- The only shell commands you may run are `lake build Rule30.Proofs.EvolveLeftDiagonalIsEventuallyPeriodicStep` and `lake env lean <file>`. Everything else is denied, and so is any shell operator — no `;`, `&&`, `|`, backticks, `$`, `>` or `<`. One bare command per Bash call.
...
- The harness verifies with a generated check theorem — `theorem harness_check : type_of% Statements.evolve_left_diagonal_isEventuallyPeriodic_step := evolve_left_diagonal_isEventuallyPeriodic_step` — against the captain's statement. So your theorem must be named exactly `evolve_left_diagonal_isEventuallyPeriodic_step` and have exactly the stated type. A weakened or generalized restatement fails, even one you could prove.
