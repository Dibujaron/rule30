...
- The only file you may edit is `Rule30/Proofs/CenterColumnNotEventuallyPeriodicOfAnyOther.lean`. Every other write is denied by a hook, not by convention.
- The only shell commands you may run are `lake build Rule30.Proofs.CenterColumnNotEventuallyPeriodicOfAnyOther` and `lake env lean <file>`. Everything else is denied, and so is any shell operator — no `;`, `&&`, `|`, backticks, `$`, `>` or `<`. One bare command per Bash call.
...
- The harness verifies with a generated check theorem — `theorem harness_check : type_of% Statements.centerColumn_not_eventually_periodic_of_any_other := centerColumn_not_eventually_periodic_of_any_other` — against the captain's statement. So your theorem must be named exactly `centerColumn_not_eventually_periodic_of_any_other` and have exactly the stated type. A weakened or generalized restatement fails, even one you could prove.
