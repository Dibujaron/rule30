...
- **"The backward extension might not be periodic."** Six mismatches at
  ...
  index left per level. `0` mismatches with slack. My notebook records this
  exact failure twice.
- **"The sharp bound `1 + max{d : P_d ≤ 2^n}` fails at `n = 22`."** It reported
  `71` against a true `m(2^22) = 55`, which would have contradicted a
  kernel-accepted theorem. The period detector accepted `2^e` without testing
  once `2^e` outgrew its window, capping every large `P_d` at the window size.
  Tight at `21` of `21` after the rewrite. **The tell was that the measurement
  contradicted a proof, not that the number looked wrong.**
- **"Flipping `b_7` makes the first cone violation land at `k > 7`."** Mine, in
  ...
  affine in `b_k` — which is the thing I was about to measure in the next
  block.
- **"`m(2^n)` can be read off the tower at index `2^n - d` for every `d`."**
  False for `d > 2^n`, i.e. small `n`: `m(2^1) = 3` needs index `-1` and came
  back as my out-of-range sentinel, which then made "all `≥ n+1`" print
  `false`. Those cells are read out of the picture instead.
- **"A coin's overshoot `K(p) - p` has mean 2."** It is `1`
  (`P(j) = 2^{-(j+1)}`), so the measured `0.995` is a match and not a
  deviation; I had the wrong null printed beside the right number for one run.
...
- **"The sweep state at the origin is a new coordinate on the centre column."**
  Already killed in my own §6 before this session; re-confirmed here from the
  other side. It is the picture's own line `x = t - k`, and the identity along
  it (C4) is the rule telescoped.
- **Nothing died for lack of depth.** Every death above has a witness computed
  rather than searched for; the recurrence was checked at 1.44M cells, the
  backward periodicity over eight configurations, the identity to `k = 2500`,
  the ANF exactly to `k = 17`, the solution counts to `2^19` words, and the
  tightness against Rowland's printed `a(n)` at every value he prints.
