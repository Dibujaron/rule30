...
the walk is frozen on about a quarter of its steps, measured to 50,000 rows and proved
for none.
-/
...
from `M` has period `q` from `N` already. So the onset of a sequence does not depend on
which of its periods is used to read it. -/
