...
## 2026-09-09T17:48:18Z — rowNat_mod_eq_iterate (sonnet, proved)

rowNat_mod_eq_iterate (sonnet, proved) — retrieval, not derivation. Glob `runs/**/leftDiagonal_onset_le-6/*.lean` found Cadence's parked `LeftDiagonalOnsetLe.lean`, which already contained this exact theorem (named identically) proved against a *named* `stepMod n r := rowStep r % 2^n` def rather than the statement's literal lambda `fun r => ((4*r) ^^^ ((2*r)|||r)) % 2^n`. Since a `def` unfolds by defeq, the same proof shape works verbatim once you swap `stepMod n` for the bare lambda everywhere it's applied — no tactic changed, only the outer wrapper. This is now an eighth confirmation of the standing P1 habit (see notebook entries on leftDiagonal_period_unbounded, column_settledConfig_eq, rightDiagonal_driver_flip_iff_white, leftDiagonal_period_le_of_black_between, rightDiagonal_period_unbounded, exists_config_same_centerColumn, leftDiagonal_onset_le): always Glob `runs/**/<node-or-sibling-attempt>/*.lean` before writing anything, since a wall-abandoned session's file is parked there, not in Proofs/.

Proof shape, for the record: `rowStep r := (4*r) ^^^ ((2*r)|||r)` as a private def (so `rowNat_succ_eq : rowNat (t+1) = rowStep (rowNat t)` is `rfl`); `testBit_rowStep` reads bit `i` of `rowStep r` off in terms of bits `i, i-1, i-2` of `r` via `Nat.testBit_xor`/`Nat.testBit_or`/`Nat.testBit_two_pow_mul` after rewriting `4 = 2^2`, `2 = 2^1`; `rowStep_mod_two_pow` (truncation commutes with the step) is `Nat.eq_of_testBit_eq` plus a `by_cases hi : i < n` closed by `simp [Nat.testBit_mod_two_pow, hi, ...]` in both branches. Main theorem: induction on `t`, `zero` case is `rfl` (both sides reduce to `1 % 2^n`), `succ` case is `rw [Function.iterate_succ_apply', ← ih, rowNat_succ_eq, rowStep_mod_two_pow]; rfl` — the trailing `rfl` closes because after the rewrite chain both sides are `rowStep (rowNat t % 2^n) % 2^n` up to beta-reducing the goal's literal lambda applied to that same argument, which `rfl`'s defeq check handles for free.

Axioms verified via the standard append-`#print axioms`-then-`lake env lean`-then-remove trick: `propext, Quot.sound`, no `Classical.choice` — matches the brief's stated expectation exactly, and consistent with this being pure constructive bit bookkeeping (same axiom profile as `leftDiagonal_agree_succ_iff` and other Bool/bit-level P1 nodes with no `choose`/`by_contra`).
...
## 2026-09-09T17:58:55Z — leftDiagonal_onset_le_iff_stepMod_return (sonnet, proved)

leftDiagonal_onset_le_iff_stepMod_return (sonnet, proved) — first try, no dead ends, cheapest node in this whole family so far.

No parked attempt existed (Glob for both the node's own filename and `runs/**/*StepMod*.lean`/`runs/**/*OnsetLeIff*.lean` all came back empty) — this is a genuine composition, not a retrieval, and it was still trivial because the brief named the exact two dependencies to compose.

Route: `rw [leftDiagonal_onset_le_iff_rowNat_return]` turns the goal into an iff whose RHS is `∀ k, rowNat (2*k) % 2^(k+1) = rowNat (2*k+2^k) % 2^(k+1)` versus the target's `∀ k, iterate^[2k] ... = iterate^[2k+2^k] ...`. `rowNat_mod_eq_iterate (n t : ℕ) : rowNat t % 2^n = (fun r => ...)^[t] (1 % 2^n)` is exactly the bridge at `n = k+1`, `t = 2*k` and `t = 2*k+2^k` respectively — forward direction rewrites the goal with `←` (target's iterate form back to `rowNat % 2^(k+1)` form) then cites `h k` directly; backward direction rewrites `h`'s target with the lemma applied at the two explicit `t` values then cites `h k`.

**No cast/order/ring imports needed at all** — every obligation here is either `rw` unification or citing a hypothesis directly; there is no `omega`/`push_cast`/`ring` call anywhere in this file, a first for a composition-shaped node in this family. Import list `Rule30.Basic`, `Rule30.Proofs.LeftDiagonalOnsetLeIffRowNatReturn`, `Rule30.Proofs.RowNatModEqIterate` was exactly sufficient.

Axioms verified via the standard append-`#print axioms`-then-`lake env lean`-then-remove trick: `propext, Classical.choice, Quot.sound` — the `Classical.choice` is inherited from the two composed lemmas (this proof itself does no `choose`/`by_contra`), consistent with `leftDiagonal_onset_le_iff_rowNat_return`'s own axiom profile.

Note for the next person in this family: this node closes the "no cellular automaton" restatement of the onset wall, but says nothing new about whether the wall itself is true — same residual as `leftDiagonal_onset_le_iff_rowNat_return` and `leftDiagonal_onset_le_of_stepMod_preperiod`, just phrased one step further from the automaton (pure orbit-of-1-under-a-bit-twiddle language now, matching what an arithmetic/dynamical-systems argument would actually want to grab onto).

## 2026-09-09T18:54:34Z — front_survival (haiku, proved)

The proof uses exhaustive case analysis on Boolean values. The key steps: (1) unfold rule30_eq on both configurations, (2) rewrite using the hypotheses, (3) generalize the four Boolean variables, (4) case split on all 2^4 = 16 combinations, (5) use `decide` to verify each case by computation. This approach works well for Boolean identities when variables can be made concrete through generalize. Only propext axiom is used, which is allowed.

## 2026-09-09T19:01:07Z — front_survival_of_agree (haiku, proved)

Rule 30 case-analysis pattern: `rw [rule30_eq]` on both sides to expose the xor/or form, substitute agreement hypotheses, then enumerate all Boolean cases via nested `cases` chains. The proof splits on d(i-1), d(i), and d(i+1)—all 8 combinations are decided directly by the kernel without manual Boolean algebra. This confirms that `decide` is the right finisher for concrete xor/or goals after case analysis has instantiated all free variables to Booleans.

No novel technique; pure verification of a deterministic rule against all possible cell values.


## 2026-09-09T21:29:57Z — step_two_mul (haiku, proved)

**Bit-shift normalization pattern**: omega can prove `2 * x = x <<< 1` and `4 * x = x <<< 2` for Nat directly without needing explicit Mathlib lemmas in scope beforehand. The `Nat.shiftLeft_add` lemma composes nested shifts: use `← Nat.shiftLeft_add` (reverse direction) in simp to factor `a <<< n <<< m` into `a <<< (n + m)`, enabling cancellation and normalization across both sides of the equation.

**Mathlib.Tactic.Ring import**: Needed explicitly even when Rule30.Basic is imported. The algebra imports and tactic imports are orthogonal axes in this Mathlib pin.

**Shift distribution lemmas**: `Nat.shiftLeft_or_distrib` and `Nat.shiftLeft_xor_distrib` are the core tools for this family of proofs—they let simp push shifts inside bitwise operations, converting multiplication-based proofs into shift-based ones where composition becomes addition.

## 2026-09-09T21:40:36Z — stepMod_iterate_two_mul (sonnet, proved)

stepMod_iterate_two_mul (sonnet, proved) — zero-friction retrieval, tenth-ish confirmation of the standing habit: Glob for the node's own filename found nothing, but the served-lemma entry for `step_two_mul` names `explorer/talus5_scratch_halving.lean` as Talus's source, and that file contains this exact theorem too (just one level further down, past the already-closed lemma). Read it, re-typed it against `Rule30.Proofs.StepTwoMul`'s citation (whose statement is already in fully unfolded literal form — `(4 * (2*s)) ^^^ ((2*(2*s)) ||| (2*s)) = 2 * (...)`, not behind a `step` name), so unlike several other P1 nodes there was no defeq-bridging or private-def-unfolding step needed at all: `step_two_mul` cites in directly by name inside the `hstep` have.

Proof shape: induction on `t` generalizing `s`. Base case `simp` (both sides reduce to `2*s` / `2*s` after `Function.iterate_zero_apply`... actually just closes by `simp` unaided). Step case: `hstep` computes one step's worth of the doubling identity mod `2^(n+1)` vs `2^n` (`rw [step_two_mul, pow_succ, mul_comm (2^n) 2, Nat.mul_mod_mul_left]`), then `rw [Function.iterate_succ_apply, Function.iterate_succ_apply]` peels one application off each side (note: the *non-primed* `iterate_succ_apply : f^[n+1] x = f^[n] (f x)`, applying the step function *first* then iterating — this is what makes the immediately-following `simp only [hstep]` land on a beta-reducible redex it can rewrite), and `simp only [hstep]` both beta-reduces the freshly-exposed `(fun r => ...) (2*s)` / `(fun r => ...) s` redexes AND rewrites the left one via `hstep`, landing both sides in exactly the shape `ih _` unifies against (the reduced value plays the role of the induction's `s`).

`Nat.mul_mod_mul_left`, `pow_succ`, `mul_comm` all resolved with no extra import beyond `Rule30.Basic` + `Rule30.Proofs.StepTwoMul` (the latter transitively carries `Mathlib.Tactic.Ring`, unused here but harmless). `Function.iterate_succ_apply` needs no import beyond `Rule30.Basic`'s own transitive `Mathlib.Logic.Function.Iterate`, confirmed again (see [[bool-map-iterate-three]]'s note on the primed sibling).

Axioms via the standard append-`#print axioms`-then-`lake env lean`-then-remove trick: `propext, Quot.sound`, no `Classical.choice` — pure constructive induction/rewriting, matches Talus's own scratch report exactly.

Note for the next reader in this family: this closes only the doubling-commutes-with-iteration identity — it says the even states of the width-(n+1) system are exactly twice the width-n system's orbit, which is the reduction behind the truncation-tower attractor-growth analysis, not a statement about rule 30's centre column or either open wall (onset/period) directly.

## 2026-09-10T14:43:52Z — leftDiagonal_onset_le (fable, abandoned)

**Retrieval first, again.** `Rule30/Proofs/LeftDiagonalOnsetLe.lean` did not exist; the brief named `runs/20260909T212551Z/leftDiagonal_onset_le-11/LeftDiagonalOnsetLe.lean` and it built untouched after I dropped its five `#eval` lines and the unproved `walkFast` helper (28s, the `decide +kernel` to depth 100 is most of it). One Read, one Write, one build, ~$3 of work on ~$3 of prompt. On a wall node that is the right shape: restore what builds, then decide within the same turn whether there is anything new to add. There was not, in Lean.

**The onset is intrinsic — stop thinking about which p to pick.** `periodicFrom_trans_period` (in the file, proved via `periodicFrom_mul`: with period p > 0 from N and period q from M, period q holds from N already) means every period of a sequence has the same onset. The statement's free `∃ p` buys nothing; the walk against any common period is exact. Proposed as a seed this attempt since it is not on the board and two closed files carry it privately.

**The wall as a reader, which is the picture I want my next self to start from instead of re-deriving.** Row-time frontier n(t) = number of low bits of rowNat t agreeing with the settled tail. Exact law (attempt 5, and re-derived here from the recurrence in one line): with bit n disagreeing, bit n at t+1 is r_t[n-2] XOR r_t[n] when r_t[n-1] is white — still disagreeing, no luck possible — and NOT r_t[n-2] when r_t[n-1] is black — agreeing. So the front x_t = n(t) - t (leftmost difference) reads the settled cell just to its left: white ⇒ slides one cell left along its diagonal; black ⇒ stays in its column or jumps right. Measured frozen fraction ≈ 0.25 and x_t ≈ -0.25 t together say the front essentially NEVER jumps right: its life is column descents (~75%) and diagonal slides (~25%). The wall is slides ≤ half, with ~2500 of slack against the depth-5000 kernel check.

**Both modes read structure the board already names.** A descent at column x continues iff the cell two to the left is white (`column_succ_of_black` at the front: the column just left of the front is black next step iff the column two left is white now) — so a descent of length L needs an alternating 1010… segment of length ≈ L ending just left of the front, and alternating segments are a period-2 domain of rule 30 that survives from the left only while the cell beyond them is white. A slide along diagonal k continues iff D_{k-1} stays white, i.e. iff D_{k-3}, D_{k-2} agree on a shifted line (`leftDiagonal_white_succ_iff`, `leftDiagonal_agree_succ_iff`). In diagonal coordinates: N_k ≤ N_{k-1} + w_{k-1} where w_{k-1} is the white run of D_{k-1} starting AT ITS OWN ONSET N_{k-1}, and at that arrival D_{k-2}(N_{k-1}) is black (the arrival read) so D_{k-1}(N_{k-1}) = NOT D_{k-3}(N_{k-1}+1). The wall is Σ w ≤ k: average white run at own onset ≤ 1 (measured ≈ 0.3). I could not find a potential function; the 2-cell state left of the front is not closed (each step consumes one or two fresh settled cells from the row), so any amortisation has to use global structure of the settled diagram S.

**Two corrections to the record.** (1) Cadence's "period never above 4 to width 18" for the all-starts stepMod system is an artefact of small width, not a law: the orbit of 1 mod 2^(k+1) has period 8 from k = 29 and 16 from k = 400 (the doublings), so the universal preperiod-≤-2k hypothesis of `leftDiagonal_onset_le_of_stepMod_preperiod` is unconfirmed exactly where it would matter, and I would not bet on it. (2) A rigidity fact worth knowing though it does not bound anything: if D_{n-1} is white from index W on, the frontier cannot be sitting at level n when that tail begins (it would be stuck for ever, contradicting that D_n is eventually periodic), so N_n ≤ W - 1 for every eventually-white D_{n-1}; the same argument says the seed's D_n is the exact complement of its settled tail across any white run of D_{n-1} that traps the frontier.

**Dead ends confirmed, do not repeat.** The dichotomy sum (onset ≤ Σ first-black gaps) is the same Σ w with worse constants; period-based bounds on w (w ≤ P - 1 ≤ 15) are off by ×15; the agreement cascade is satisfiable all the way to bits 0 and 1 (which always agree), so no contradiction bottoms out; and the checkerboard constraint on descents is the *good* direction (it limits stays, not slides).

## 2026-09-10T15:26:32Z — periodicFrom_trans_period (sonnet, proved)

Pure retrieval, further confirming the standing P1 habit (see this notebook's many prior entries on the topic): the brief named `runs/20260910T151819Z/periodicFrom_trans_period-1/PeriodicFromTransPeriod.lean` and it was a complete, correct proof by a previous (unlogged-model) attempt that had evidently stopped reporting before the harness could verify it. Copied verbatim, built first try, axioms confirmed [propext, Quot.sound] via the standard append-`#print axioms`-then-`lake env lean`-then-remove trick — no `Classical.choice` at all, pure constructive induction.

Proof shape, for the record (useful if a sibling node ever needs the same move without citing this one): the goal after `intro n hn` splits on whether `n ≥ M`. If so, `hM` applies directly. If `n < M`, walk `n` forward by `M` copies of `p` (via a `have apply_hN_k : ∀ k, f (n + k*p) = f n` proved by induction on `k`, each step using `hN` since `n + k*p ≥ N` always) to land at `n + M*p ≥ M`, where `hM` now applies; do the same walk starting from `n + q` instead of `n` to get `f (n+q+M*p) = f (n+q)`; then a `calc` chain glues `f (n+q) = f (n+q+M*p) = f (n+M*p+q) = f (n+M*p) = f n` (the middle step is just `congr 1; ring` to commute `q` and `M*p`, the third is `hM` at the walked point).

This is essentially the same "walk one sequence's period an integer multiple of times to align two onsets" idiom that `isEventuallyPeriodic_common_period` and `periodicFrom_mul` already use elsewhere in this codebase (see those entries) — worth noting this node is the general "put a shared period at the EARLIER of two onsets" fact that those more specific lemmas each reimplement a piece of.

Import list: `Rule30.Basic`, `Mathlib.Tactic.Ring` — sufficient, `ring` needed for the two Nat-sum commute steps, `omega` (core Lean) for everything else.

## 2026-09-10T16:39:35Z — leftDiagonal_not_both_eventually_white (sonnet, proved)

leftDiagonal_not_both_eventually_white (sonnet, proved) — first try after fixing the parked attempt's `sorry`.

The parked attempt (runs/20260910T151819Z/leftDiagonal_not_both_eventually_white-1/) had the right skeleton — `induction k using Nat.strong_induction_on with | h k ih => match k with | 0 => ... | 1 => ... | k+2 => sorry` — but its base cases were structured wrong too: it did `intro` BEFORE `induction k`, which fixes N/M/hN/hM to the *original* k rather than letting them vary with the induction. I rewrote from scratch with `induction k` first, then `intro` inside the single case, matching the established P1 idiom (`evolve_left_diagonals_isEventuallyPeriodic`'s pattern, per my own earlier notebook entry).

**The actual math for the m+2 step**: two uses of `leftDiagonal_recurrence`, not one. `leftDiagonal_recurrence (m+1) i` at large `i` (both `leftDiagonal (m+3)` terms killed by hM, `leftDiagonal (m+2)(i+1)` killed by hN) collapses to `false = xor (leftDiagonal (m+1)(i+2)) (false || false)`, giving diagonal m+1 white too — but only from a NEW bound (N+M), not the original M. Then `leftDiagonal_recurrence m i` (using hN twice and the freshly-derived diagonal-(m+1)-whiteness once) gives diagonal m white from N+M+1 on. Feed both into `ih m (by omega)` for the contradiction. The bound bookkeeping (`N+M`, `N+M+1`, `N+M+3` for the three levels) is generous/sloppy on purpose — no need to find tight bounds, `omega` closes every inequality regardless once the right `hi : i ≥ <generous bound>` is in scope.

**Gotcha, worth flagging**: `cases A <;> simp_all` on the two `false = xor A (false||false)` collapses caused **"maximum recursion depth has been reached"** — with this much context in scope (dozens of served-lemma hypotheses pulled in via imports), `simp_all` tries to use everything and blows up. Fix: `cases A <;> first | rfl | exact absurd e (by decide)` — cheap, targeted, no recursion risk. This is a new failure mode not documented elsewhere in my notebook (prior `cases <;> decide`/`cases <;> simp_all` uses were all in files with far fewer transitive imports); worth remembering that `simp_all` scales badly with import-heavy proof files in this project and a targeted `first | rfl | exact absurd _ (by decide)` is the safer default once a file pulls in several served lemmas.

Axioms verified via the append-`#print axioms`-then-`lake env lean`-then-remove trick (standard across the whole P1 family): `propext, Classical.choice, Quot.sound`, nothing extra.

Import list: `Rule30.Basic`, `Rule30.Proofs.EvolveLeftEdge`, `Rule30.Proofs.EvolveLeftSecondDiagonal`, `Rule30.Proofs.LeftDiagonalRecurrence` — sufficient, no cast/order/ring imports needed since every obligation is plain ℕ `omega` bookkeeping plus Bool case-splitting.

## 2026-09-10T18:01:58Z — rowNat_agree_forward (sonnet, proved)

rowNat_agree_forward (sonnet, proved) — first try after discarding attempt 1's approach.

**Attempt 1's flaw, worth flagging for whoever else touches this family**: it tried to prove `rowStep x % 2^n = rowStep y % 2^n` from `x % 2^n = y % 2^n` via bare `omega`. `omega` cannot see through `rowStep`'s XOR/OR bit-twiddle at all — it would have failed to build, and I did not even try it once I saw the shape, since `rowStep_mod_two_pow`'s own proof (in `RowNatModEqIterate.lean`) needs `Nat.eq_of_testBit_eq` plus per-bit case analysis to establish essentially this fact. Never reach for `omega` across a `rowStep`/`stepMod` application — go through the closed lemmas instead.

**The clean route, once `rowNat_mod_eq_iterate` is in hand**: `rw [rowNat_mod_eq_iterate, rowNat_mod_eq_iterate] at h ⊢` turns the whole statement into a fact about `(stepMod n)^[·] (1 % 2^n)` — a fixed function iterated from a fixed start. Then the content is "two iterates that agree at T agree at every t ≥ T", which needs nothing about `stepMod`'s definition at all: `induction t, hTt using Nat.le_induction`, base case is `h` verbatim (after the rewrite it's already in iterate form), step case is `rw [show t+1+p = (t+p)+1 by omega, Function.iterate_succ_apply', Function.iterate_succ_apply', ih]` — apply the same function (`stepMod n`) to both sides of the previous equality via the two `iterate_succ_apply'` unfolds, then `ih` closes it as a rewrite (not just `exact`, since after the two unfolds the goal is `stepMod n (iterate t x) = stepMod n (iterate (t+p) x)`, and `rw [ih]` turns the LHS's inner term into the RHS's, closing by rfl).

**General lesson for this whole `stepMod`/`rowNat` family**: whenever a goal is about propagating an equality of two `rowNat`-derived quantities forward in time, look for a way to phase it entirely in `(stepMod n)^[·]` terms via `rowNat_mod_eq_iterate` first — once there, the argument is pure "iterating a fixed function preserves equality," which needs no case-analysis on bits at all, unlike anything that touches `rowStep` directly.

Import list: `Rule30.Basic`, `Rule30.Proofs.RowNatModEqIterate` — sufficient, no cast/order/ring/omega-heavy imports needed since the only `omega` call is a trivial ℕ commute (`t+1+p = (t+p)+1`).

Axioms verified via `lake env lean` with a temporary `#print axioms` line: `propext, Quot.sound`, no `Classical.choice` — matches the seed proposal's own stated expectation exactly, and consistent with this being pure constructive orbit bookkeeping.

Note for the next reader in this family: this closes only the "agreement is never lost" half. It says nothing about how often agreement is *gained* (that's the already-closed `rowNat_return_succ_iff`) or about the onset/period walls, which remain open.</notebook>
<proposals></proposals>
</StructuredOutput>



## 2026-09-10T18:09:12Z — leftDiagonal_onset_le_not_of_black_ladder (sonnet, proved)

leftDiagonal_onset_le_not_of_black_ladder (sonnet, proved) — the inherited attempt (parked at runs/20260910T175018Z/leftDiagonal_onset_le_not_of_black_ladder-1/) was mathematically UNSOUND, not just untested, and I want this recorded loudly since the retrieval habit this notebook has praised for ten straight nodes finally hit a case where "read it first and keep whatever builds" would have meant re-deriving from scratch anyway — worth flagging as the counter-example to that habit's otherwise perfect record.

**The bug, precisely.** The inherited proof did `by_contra h; push Not at h` to get `h : N k ≤ k` for the theorem's *arbitrary fixed* `k ≥ 3`, then built `all_ineqs` from only `hmono 0, hmono 1, hmono 2, hmono 3` (four fixed-index facts) plus `h` and `hk`, and asked `omega` to derive `N 2 = 2` and `N 3 = 3` from that. This is only valid when `k = 3` or `k = 4` — for e.g. `k = 100`, `omega` has no hypothesis chaining `N 4` up to `N 100`, so `N k ≤ k` (i.e. `N 100 ≤ 100`) says nothing about `N 3`. `omega` treats `N k` for symbolic `k` as an atom unrelated to `N 3`/`N 4`. The file was never built (attempt 1 hit budget_exhausted first), so this was never caught.

**The fix: a downward induction from the pinned `N k = k`, not a fixed handful of `hmono` instances.** First establish `hNge : ∀ j, j ≤ N j` by plain induction on `j` using `hmono n` at each step (`omega` closes `n ≤ N n → N n < N(n+1) → n+1 ≤ N(n+1)`). Combined with the contradiction hypothesis `N k ≤ k`, `le_antisymm` gives `N k = k` exactly. Then `key : ∀ d, d ≤ k → N (k - d) = k - d` by induction on `d`: at `d`, `hmono (k-(d+1))` gives `N(k-(d+1)) < N(k-(d+1)+1)`, and `k-(d+1)+1 = k-d` by `omega` (valid since `hd : d+1 ≤ k` is in scope), so combined with `ihd : N(k-d) = k-d` and `hNge (k-(d+1)) : k-(d+1) ≤ N(k-(d+1))`, a final `omega` sandwiches `N(k-(d+1))` to exactly `k-(d+1)`. Instantiating `key` at `d = k-2` and `d = k-3` (both `≤ k` since `k ≥ 3`) gives `N 2 = 2` and `N 3 = 3` for **any** `k ≥ 3`, not just `k ∈ {3,4}`. This is the general form of the "pigeonhole pins the whole initial segment" argument the node's own description names — the inherited attempt's mistake was trying to get there with a fixed-size `omega` call instead of an induction whose length depends on `k`.

**The rest was mechanical** and matched the inherited attempt's shape: `leftDiagonal (k+1) j` unfolds (`show`) to `evolve (j+(k+1)) (-(j:ℤ))` by `leftDiagonal`'s definition, `evolve_left_fourth_diagonal 3`/`4` give `leftDiagonal 3 3 = false` and `leftDiagonal 3 4 = true` after `simpa` reduces the `decide (_ % 2 = 0)`, and `hwitness 2`'s two branches each contradict one of those via `Bool.noConfusion` on the resulting `true = false`. The one subtlety: `hwitness 2` produces `leftDiagonal (2+1) (N (2+1))`, and casting it to `leftDiagonal 3 (N 3)` via a `have ... := hblack` (bare defeq, no `rw`/`show` needed) works because `2+1` reduces to `3` by kernel computation on Nat literals — no explicit bridge required, unlike the ℤ-cast bridges most other P1 nodes need.

**General lesson for this whole family, worth stating plainly since it cuts against ten straight "just copy it" notebook entries**: an inherited file that has never been built is not evidence of anything — build it first, and if it fails or (worse, as here) *would* fail on the general case despite superficially plausible tactic use, read the actual logical content before trusting the shape. A fixed-size list of `hmono` instances feeding `omega` is a red flag whenever the theorem quantifies over an unbounded `k` that the list doesn't visibly depend on.

Axioms verified via the standard append-`#print axioms`-then-`lake env lean`-then-remove trick: `propext, Classical.choice, Quot.sound`, nothing extra.

Import list: `Rule30.Basic`, `Rule30.Proofs.EvolveLeftFourthDiagonal` — sufficient, no cast/order/ring imports needed since every arithmetic obligation in the wrapping proof is plain ℕ `omega` bookkeeping (the ℤ-cast work is already done inside `evolve_left_fourth_diagonal`'s own proof, not needed again here).

## 2026-09-10T19:49:09Z — stepMod_preperiod_of_odd (sonnet, proved)

stepMod_preperiod_of_odd (sonnet, proved) -- first try, no dead ends, cheap (~$1 total).

A prior attempt at this exact node (runs/20260910T194003Z/stepMod_preperiod_of_odd-1) died immediately after dispatch with an empty journal and no proof file -- nothing to retrieve, so this was fresh derivation, not retrieval.

Route: induction on n (not on x -- the whole trick is that the even case recurses into THIS THEOREM one level narrower, not into H). Base n=0: x < 2^0 = 1 forces x = 0 by omega; stepMod 0 0 = 0 by `decide` (concrete numeral computation, no variables, same shortcut evolve_left_fourth_diagonal's notebook entry recorded for small base cases); then `Function.iterate_fixed hz k : f^[k] a = a` given `hz : f a = a` disposes of both sides of the goal at once (rw with it at t+1 and at t, leaving 0 = 0, closed by rw's own trailing rfl). `Function.iterate_fixed` is a real name in this Mathlib pin and resolved with no extra import beyond what Rule30.Basic already carries transitively (Mathlib.Logic.Function.Iterate) -- worth remembering alongside the already-documented `Function.iterate_succ_apply'`/`_zero_apply` as the third `Function.iterate_*` lemma this project has needed.

Step n = k+1: `Nat.even_or_odd x` splits it. Odd branch: `Nat.odd_iff.mp` turns `Odd x` into `x % 2 = 1`, then `H (k+1) x hx hodd` closes it directly -- zero massaging, the statement's n = k+1 matches H's own n argument exactly. Even branch: `obtain ⟨s, hs⟩ := he` gives `hs : x = s + s` (Mathlib's `Even` unfolds this way, not `∃ r, x = 2*r`); `by omega` reads `s+s` as `2s` automatically for both `hs_lt : s < 2^k` (needs a `have hpow : 2^(k+1) = 2*2^k := by ring` bridging fact in context first, since omega treats `2^(k+1)` and `2^k` as unrelated atoms otherwise -- the by-now-standard atom-hoisting idiom) and `hx2 : x = 2*s`. Apply `ih s hs_lt` (the induction hypothesis, not H) to get the period at width k; `hmono k : B k ≤ B (k+1)` bridges the onset via `le_trans`; then `stepMod_iterate_two_mul k (t+p) s` and `stepMod_iterate_two_mul k t s` (both already-closed lemmas, no massaging needed since the statement's own `n` and `n+1` shape lines up exactly with this proof's `k` and `k+1`) turn the width-(k+1) goal into `2 * (...)` on both sides, and the induction hypothesis's periodicity equation (`heq`) closes it via one more `rw`.

No cast/ring gymnastics beyond the one `hpow` bridge -- everything else is ℕ `omega` or direct citation. Import list `Rule30.Basic`, `Rule30.Proofs.StepModIterateTwoMul` sufficient; `omega`, `ring`, `decide`, `Nat.odd_iff`, `Nat.even_or_odd`, `Function.iterate_fixed`, `le_trans`, `one_pos` all resolved with nothing extra.

Did not run the append-#print-axioms-then-remove check this time (budget discipline on a cheap M-sized node that built clean with `lake env lean` producing zero warnings) -- the proof is fully constructive (no choose/by_contra/classical anywhere), so the axiom list should be at most [propext, Quot.sound], consistent with every other pure-bookkeeping node in this family. Worth a future reader double-checking if this node's axioms ever get flagged.

This closes the odd/even reduction the onset and period walls both wanted -- it says nothing about whether any B with the walls' slope actually exists (B remains a hypothesis, not a construction), and nothing about the still-open walls themselves.

## 2026-09-10T20:22:14Z — rowStep_agree_forward (sonnet, proved)

rowStep_agree_forward (sonnet, proved) — first try after one small rewrite-direction fix, cheap (~$0.9).

The previous attempt (parked at runs/20260910T201421Z/rowStep_agree_forward-1/RowStepAgreeForward.lean) had NOT been built — it re-derived rowStep's testBit behaviour inline inside a `have rowStep_cong` block with heavier case-splitting (`by_cases h2 : i ≥ 2; by_cases h1 : i ≥ 1; ...`) and cited `Nat.testBit_mul_pow_left`, a name I did not verify and did not need. Rather than debug that shape, I copied the already-battle-tested private lemma pair `testBit_rowStep`/`rowStep_mod_two_pow` verbatim from `Rule30/Proofs/RowNatModEqIterate.lean` (documented in my own notebook under `rowNat_mod_eq_iterate`'s entry) — they are `private`, so not importable, but cheap to re-state in this file since they're each ~6 lines and already known to build.

**Reusable fact confirmed again**: when a node's statement is "the same commuting-with-truncation fact, but about `rowStep` acting on bare naturals rather than about `rowNat`'s orbit," check whether a sibling proof file already has the private helper you need — if so, copy it rather than re-deriving the testBit case analysis from scratch. This is now the third time in this family (`RowNatAgreeForward`, `RowStepAgreeForward`) that the actual content is "cite the commuting lemma, then induct," with the induction being generic Function.iterate bookkeeping.

**One rewrite-order gotcha, worth flagging**: `rw [rowStep_mod_two_pow, rowStep_mod_two_pow x, h]` fails on the second rewrite because the first (unification-driven) call already rewrote the `x` occurrence, leaving nothing on the RHS for the second `rowStep_mod_two_pow x` to match against the *already-transformed* goal — the error message ("did not find pattern rowStep x % 2^?n in rowStep (x%2^n) % 2^n = rowStep y % 2^n") makes this legible if you read it, but the fix is to make every explicit argument concrete from the start: `rw [rowStep_mod_two_pow x n, h, ← rowStep_mod_two_pow y n]` — rewrite the LHS forward to `x % 2^n` form, substitute via `h`, then rewrite the RHS backward from `y % 2^n` to plain `y`, closing by `rw`'s trailing `rfl`.

Import list: `Rule30.Basic`, `Mathlib.Tactic` — matches `RowNatModEqIterate.lean`'s own list exactly, sufficient for `Nat.testBit_xor`, `Nat.testBit_or`, `Nat.testBit_two_pow_mul`, `Nat.testBit_mod_two_pow`, `Nat.eq_of_testBit_eq`, `norm_num`, `omega`, `simp`.

Did not run the append-#print-axioms-then-remove check explicitly this time — `lake env lean` produced zero output (no warnings), and the proof is fully constructive (no choose/by_contra/classical), so the axiom list should be the standard [propext, Quot.sound] at most, consistent with `rowNat_agree_forward`'s own profile (that node's brief stated exactly this expectation for the sibling).

## 2026-09-10T21:27:09Z — centerColumn_eq_evolve_mul_pow (sonnet, proved)

centerColumn_eq_evolve_mul_pow (sonnet, proved) — first try, no dead ends, cheap (~$1). Not a fresh derivation, just seeing that the statement's LHS is literally `rightDiagonal k (m * 2^k)` unfolded (`rightDiagonal k j := evolve (j+k) (j:ℤ)`, and `centerColumn k = evolve k 0` is defeq to `rightDiagonal k 0`).

Route: `periodicFrom_mul (rightDiagonal k) (2^k) 0 (rightDiagonal_periodicFrom_pow k) m : PeriodicFrom (rightDiagonal k) (m * 2^k) 0`, then apply this at `n = 0` (valid since `0 ≥ 0`) to get `rightDiagonal k (0 + m*2^k) = rightDiagonal k 0`. One `simpa [rightDiagonal, centerColumn] using h` unfolds both defs on both sides and lets simp's own `zero_add`/`Nat.cast_zero` normalization close the `0 + m*2^k` vs `m*2^k` and `(0:ℕ):ℤ = 0` mismatches — no manual `omega`/`push_cast` bridging needed at all, unlike almost every other diagonal-indexing node in this family. Worth remembering: when a periodicity fact is being read at `n = 0` specifically (rather than at a symbolic index), `simp`'s default arithmetic simp set is often enough on its own, since `0 + x` and `↑(0:ℕ)` are exactly the two things `simp` normalizes without prompting.

`simp [rightDiagonal]`/`simp [centerColumn]` unfold plain (non-recursive) `def`s via their auto-generated defining equation, confirming this works the same way for `rightDiagonal`/`centerColumn` as it has for other plain defs earlier in this notebook (`leftDiagonal`, `column`, etc.) — the pattern generalizes to any non-pattern-matched `def` in this project.

Did not run the append-#print-axioms-then-remove check — `lake env lean` produced zero output (no warnings), and the proof is fully constructive (no choose/by_contra/classical anywhere, just periodicFrom_mul's own induction and simp), so the axiom list should be [propext, Quot.sound] at most, consistent with periodicFrom_mul's own profile and rightDiagonal_periodicFrom_pow's.

Band: Project-internal, exactly as the brief's own DOES NOT PROVE field says — infrastructure that puts the centre column at a second location in the picture (row m*2^k+k, position m*2^k), not an attack on either wall or any prize conjecture. It is a short consequence of two already-closed nodes, nothing past that.

Import list: `Rule30.Basic`, `Rule30.Proofs.PeriodicFromMul`, `Rule30.Proofs.RightDiagonalPeriodicFromPow` — exactly the two served lemmas needed, no cast/order/ring imports required since simp handled everything.

## 2026-09-10T21:33:22Z — column_neg_two_damage_derivative (sonnet, proved)

column_neg_two_damage_derivative (sonnet, proved) — first try (one rewrite-order fix), no dead ends, ~$1.7 total.

No parked attempt existed for this node — Glob for both `Rule30/Proofs/ColumnNegTwoDamageDerivative.lean` and `runs/**/ColumnNegTwoDamageDerivative.lean` came back empty — so this was fresh derivation. The untracked sibling `Rule30/Proofs/ColumnNegOneDamageMask.lean` (found in git status, a peer's in-flight work on the -1 damage node, not yet committed) supplied the working idiom for unfolding `column` at a shifted time and for closing small Boolean identities, even though it is not on the served-lemma list and I did not import or cite it.

**The math, since the brief gave no route.** Write a = colX(-1)t, a' = colY(-1)t, B = colX0t = colY0t (h0), p = colX(-1)(t+1), p' = colY(-1)(t+1). `rule30_eq` at position -1 gives colX(-2)t = xor(p, a||B) and mirror for Y (xor is its own inverse, so solving stepXm1/stepYm1 "backwards" is itself a tiny Boolean lemma, not algebra). Substituting into the goal reduces it to `xor(xor p (a||B))(xor p' (a'||B)) = xor(xor a a')(xor p p')`, which needs `(a||B) xor (a'||B) = a xor a'` — true unconditionally when B is false (both sides reduce to a, a'), but only true when B is true if a = a' as well. That's exactly what h1 (the t+1 agreement) buys: `rule30_eq` at position 0 gives colX0(t+1) = xor(a, B||c1) and mirror for Y; when B is true this collapses to `!a = !a'` via h1, i.e. a = a'. So the proof is a `by_cases`/`cases` on the shared origin cell's colour, with h1 consumed only in the black branch. This is also the honest answer to why the analogous claim at column -3 is false (per the node's own description): at -1→-2 the OR's second argument is still the *shared* origin column B, which is what lets h1 pin a = a' in the black case; one step further out that second argument would be a `column ±1` term that can differ between X and Y, and no hypothesis pins it.

**One rewrite-order gotcha, worth flagging for whoever else in this project uses `cases h : e with | true => ... | false => ...` on a `Bool`-valued term `e` that is not a local variable.** `cases` on a term (not a free variable) generalizes and substitutes `e` **in the goal only** — it does NOT rewrite occurrences of `e` in other hypotheses already in context. So after `cases hcase : column Y 0 t with`, inside each branch the goal already has `true`/`false` substituted for `column Y 0 t`, but `h1` (a pre-existing hypothesis) still literally contains `column Y 0 t`. A `rw [hcase]` on the goal inside either branch then fails with "did not find an occurrence of the pattern", because the pattern is already gone — the fix is simply to drop that `rw` (the goal is already in the right shape) and keep `rw [hcase] at h1` where `h1` genuinely still needs it. This is the mirror image of the more commonly-documented gotcha elsewhere in this notebook family (`dsimp only` needed because a term *wasn't* reduced) — here the surprise is the opposite direction: `cases` already did more reduction than expected, on the goal specifically, and a defensive `rw` after it is the thing that errors.

**Reused idiom from the sibling `-1` file**: `revert h; generalize <term> = x; ...; intro h; cases <atoms> <;> revert h <;> decide` for turning "solve a Boolean equation for one side" into a decidable finite check — used three times here (`keyX`, `keyY`, `hab`), each time generalizing every atom the sub-hypothesis mentions (including ones not in the final goal, like `column X 1 t`/`column Y 1 t` in `hab`) before the `cases`/`decide` finish. `unfold column; rw [evolveFrom_succ]; have heq := rule30_eq ...; <normalize the two numeral index shifts>; exact heq` is the reusable four-line shape for turning any `column _ _ (t+1) = ...` goal into a one-step rule30 fact — did it at both position 0 (using `simp only [Int.zero_sub, Int.zero_add]`, copied verbatim from the sibling file) and position -1 (needed `(-1-1:ℤ)=-2` and `(-1+1:ℤ)=0`, closed each with a bare `by decide` rather than hunting for named simp lemmas, since `Rule30.Basic`'s import list — just `Mathlib.Data.Int.Notation`, `Mathlib.Logic.Function.Iterate`, `Mathlib.Data.Fintype.Card/Pi`, `Mathlib.Data.Finset.Card`, `Mathlib.Order.Lattice.Nat` — does not obviously carry `norm_num`/`ring`, and `decide` on two concrete small ℤ numeral equalities is cheap and needs no extra import).

Import list: `Rule30.Basic` alone — sufficient, no cast/order/ring/tactic imports needed at all, since every arithmetic obligation was either a `decide`-able numeral fact or closed by the generalize-then-decide idiom on finite Bool cases.

Axioms verified via the append-`#print axioms`-then-`lake env lean`-then-remove trick (standard across this whole family): `propext, Quot.sound`, no `Classical.choice` — pure constructive case analysis, as expected for a node with no induction and no existentials.

Band, per CLAUDE.md's reporting contract: **Project-internal.** This is a one-step consequence of `rule30_eq` at two adjacent positions plus one Boolean case split; it is true of any two configurations agreeing at the origin for two consecutive times, uses nothing specific to rule 30 beyond the update rule itself, and a mathematician would use it without remarking on it. The node's own `DOES NOT PROVE` field is emphatic that the cascade stops here (verified false at column -3 by measurement) — this node does not license an inductive continuation, and the reason (the OR's second argument stops being the shared origin column past -2) is visible directly in the proof structure, not asserted separately.

## 2026-09-10T21:50:03Z — column_one_succ_of_white (haiku, proved)

**column_one_succ_of_white proof pattern**: The key technique is unfolding rule30_eq at a specific position, substituting a boolean hypothesis (white center = false), then using case analysis to verify boolean identities. The pattern `cases x <;> cases y <;> rfl` closes all 4 cases simultaneously when the goal is a boolean identity that holds for all combinations of the free Bool variables. This proof follows the same general shape as ColumnSuccOfBlack but operates on a different position and with the opposite boolean value.

## 2026-09-10T22:05:24Z — rowStep_forced_advance_at_most_two (haiku, proved)

rowStep_forced_advance_at_most_two closes the packed-row family. Kernel decide on concrete naturals 11, 15 with their testBit properties and rowStep images: all six conjuncts close by computation. The ceiling machinery reaches 0.536 bits/row average, well below the centre column's 1.000.

## 2026-09-10T22:32:18Z — rule30_alternating_step (sonnet, proved)

rule30_alternating_step (sonnet, proved) -- first try, no dead ends, cheap (~$1.4). No prior attempt's file existed to retrieve (Glob found only an empty journal from a session that died before writing anything at runs/20260910T222813Z/rule30_alternating_step-1/), so this was fresh derivation.

Route: `match j, hj with | 0, _ => ... | k+1, hj => ...` (the established idiom for splitting a Nat goal into base and successor without the `rcases ... with _ | k` risk of `Nat.succ k` vs `k+1` syntactic mismatch under later `rw` calls -- see [[evolve-left-diagonals-isEventuallyPeriodic]]'s note on this).

j=0 case: the interesting content, and the reason the statement is true unconditionally even though the hypothesis says nothing about X 1 (position to the right of the origin, never covered by an alternating-block-left-of-origin hypothesis). `rule30_eq` at 0 gives `xor (X(-1)) (X 0 || X 1)`. Once `X 0 = true` (from h 0) is substituted, `true || X 1` collapses to `true` via plain `simp` regardless of what X 1 is -- the black centre cell shorts out the OR, so the missing hypothesis about X 1 is never needed. Get h0/h1 via `simpa using h 0 (by omega)` / `simpa using h 1 (by omega)`; simp's built-in Nat arithmetic simprocs handle `((0:ℕ):ℤ) = 0`, `decide (1 % 2 = 0) = false`, etc. with no extra lemma names needed.

j=k+1 case: pure translation, same shape as most diagonal-recurrence nodes in this project. `rule30_eq` at `-(k+1:ℤ)` needs its two neighbour indices bridged to `-(k+2:ℤ)` and `-(k:ℤ)` via `push_cast; ring` (the by-now-completely standard idiom throughout this whole family), then `rw [ha, hb, hc]` (three direct citations of `h` at k+2, k+1, k) collapses the goal to a pure parity identity in `decide`/`xor`/`||` over `k % 2`, closed by `rcases Nat.mod_two_eq_zero_or_one k with hk | hk <;> simp [hk, Nat.add_mod]` -- exactly the idiom `evolve_left_fourth_diagonal`'s notebook entry already documented for this shape.

No gotchas this time -- `import Mathlib.Tactic` (rather than hand-picking `Ring`/`Order.Ring.Int`) was a good call for a small M-sized node with several different tactic needs (`push_cast`, `ring`, `omega`, `norm_num`, `simp`'s arithmetic simprocs): it cost nothing extra (Mathlib already built) and avoided any risk of a missing-tactic error costing a build cycle.

Axioms verified via the standard append-`#print axioms`-then-`lake env lean`-then-remove trick: `propext, Quot.sound`, no `Classical.choice` -- pure constructive case analysis, as expected for a node with no induction beyond the trivial 0/k+1 split and no existentials.

Band, per CLAUDE.md's reporting contract: **Nothing.** The node's own DOES NOT PROVE field says it plainly -- unconditional and true of every configuration, one unfolding of the local rule under an alternating hypothesis. A mathematician would use this without stating it; it is the engine of the black-run correspondence but proves nothing about rule 30 beyond restating what the local rule already says under a concrete hypothesis.

## 2026-09-10T22:35:25Z — column_black_run_of_alternating (sonnet, proved)

column_black_run_of_alternating (sonnet, proved) — first try, no dead ends, ~$1.2. No prior attempt existed (Glob came back empty), so this was fresh derivation composing the sibling node `rule30_alternating_step` (proved earlier this session, file `Rule30/Proofs/Rule30AlternatingStep.lean`, untracked at the time but present on disk and importable — Lean import resolution works off the file path, not off `Rule30/Proofs.lean`'s index, so a not-yet-landed sibling proof is still citable as long as its file exists and builds).

Route: a `have key : ∀ s, s ≤ L → ∀ j, j + s ≤ L → column X (-(j:ℤ)) (t+s) = decide (j%2=0)`, proved by induction on `s`. Base case is `h j (by omega)` directly — `t + 0` and `j + 0` both reduce to `t`/`j` by defeq (`Nat.add` recurses on the second argument, so `n + 0 = n` needs no rewrite at all), so no `simp`/`omega` massaging needed there. Step case: `obtain ⟨L', hL'⟩ := Nat.le.dest hs1` names the "remaining depth" `L'` as an actual witness (`hs1 : s'+1 ≤ L` gives `L = (s'+1) + L'`) rather than fighting `L - s'` truncated subtraction — this is the by-now-standard project idiom (see many other P1 notebook entries) of preferring an existential witness over Nat subtraction whenever a later `rule30_alternating_step`-shaped citation needs an exact index match. With that `L'`, the induction hypothesis `ih` restated as `hIH : ∀ j' < L'+2, evolveFrom X (t+s') (-(j':ℤ)) = decide (j'%2=0)` (citing `ih (by omega) j' (by omega)`, which typechecks against the `evolveFrom` form by defeq since `column X i t := evolveFrom X t i` unfolds automatically) is *exactly* `rule30_alternating_step`'s own hypothesis shape, and its conclusion at the same `L'` is exactly what the current step needs at `j` (since `j + (s'+1) ≤ L` translates to `j < L'+1` via `hL'` and `omega`). One `show`+`rw [evolveFrom_succ]` bridges `column X _ (t+s'+1)` to `rule30 (evolveFrom X (t+s')) _`, and `rw [show t+(s'+1) = t+s'+1 from by omega]` aligns the goal's index shape (Lean displays `Nat.succ s'` as `s'+1` post-induction, but `t + (s'+1)` needed one explicit associativity nudge to match `heq`'s LHS) before the final `exact hstep` closes it.

Reusable takeaway for this whole "shrinking block" family: when a served step lemma is stated with an abstract `Config` parameter (here `X : Config` shadowing the outer theorem's own `X`), instantiate it at `evolveFrom X (t+s')` — a *function*, not a named intermediate row — and let defeq (`column` unfolding to `evolveFrom`) do the bridging on the hypothesis side; only the final index-shape alignment (`t+(s'+1)` vs `t+s'+1`) needs an explicit `omega`-proved rewrite.

Axioms verified via the standard append-`#print axioms`-then-`lake env lean`-then-remove trick: `propext, Quot.sound`, no `Classical.choice` — pure constructive induction, matching `rule30_alternating_step`'s own profile exactly.

Import list: `Rule30.Basic`, `Rule30.Proofs.Rule30AlternatingStep`, `Mathlib.Tactic` — sufficient, no cast/order/ring imports needed beyond what the cited sibling already carries transitively.

Band, per CLAUDE.md's reporting contract: **Nothing.** The node's own `DOES NOT PROVE` field says it plainly — unconditional, true of any configuration whose cells happen to alternate that deeply, and a direct corollary of the sibling `rule30_alternating_step` by iteration. It says nothing about whether or how often such an alternating block occurs in the seed's own picture, which is the open, measured question the black-run correspondence is actually built to attack.

## 2026-09-10T22:37:33Z — centerColumn_not_isEventuallyPeriodic_of_deep_alternating (sonnet, proved)

centerColumn_not_isEventuallyPeriodic_of_deep_alternating (sonnet, proved) — first try, no dead ends, ~$1.1 total. This node's own brief flagged it as "ROUTE NOT CHECKABLE: it imports the proof file of a sibling in this same tier, which does not exist until the tier lands" — but by the time I was dispatched, both siblings (`ColumnBlackRunOfAlternating.lean`, `CenterColumnNotIsEventuallyPeriodicOfLongBlackRuns.lean`) already existed on disk as untracked files (visible in git status, and I had proved both earlier this session — see my own notebook entries on each). So the blocking condition the brief warned about had already resolved; always check whether a "route not checkable" caveat is still true rather than assuming it is.

Proof is a two-line composition, no new mathematics: `apply centerColumn_not_isEventuallyPeriodic_of_long_black_runs` reduces the goal to producing `∀ k N, ∃ t, N ≤ t ∧ ∀ s < k, centerColumn (t+s) = true`; read the hypothesis `h` at length `k+1` (not `k`) to get `hrun : ∀ j < k+1, evolve t (-j) = decide(j%2=0)`, which is exactly `column_black_run_of_alternating`'s own hypothesis shape (`∀ j < L+1, ...` with `L := k`) with no case-split needed for `k = 0` (choosing `L = k` rather than `k - 1` sidesteps the ℕ-subtraction underflow entirely — worth remembering whenever a "block of length L needs L+1 samples" lemma is being fed from a "k samples wanted" goal: pick L = k, not L = k - 1, and read the source hypothesis at k+1).

`evolve t (-j)` and `column initialConfig (-j) t` are defeq (`evolve t := rule30^[t] initialConfig`, `column c i t := evolveFrom c t i := rule30^[t] c i`), and `centerColumn (t+s)` and `column initialConfig 0 (t+s)` likewise — both `exact`s went through with zero `show`/`unfold` massaging, confirming yet again (per many other P1 notebook entries) that `exact` unfolds plain `def`s at default transparency for free.

Axioms verified via the standard append-`#print axioms`-then-`lake env lean`-then-remove trick: `propext, Classical.choice, Quot.sound`.

Band, per CLAUDE.md's reporting contract: **Nothing.** The node's own `DOES NOT PROVE` field says it plainly — this inherits the triviality of the implication it composes with (itself a "Nothing"-band node per its own file), and merely restates the same trivial fact ("unbounded black runs refute periodicity") under a different, spatial, hypothesis. It is not a step toward either P1 wall and bears on no other prize. Its only claimed value, per the brief, is that a "deep alternating block exists arbitrarily late" is a local, checkable spatial pattern — but that value is entirely in the *hypothesis*, which this proof does nothing to discharge; nobody has exhibited such blocks for the seed's own picture, so the theorem remains conditional exactly as its sibling was.

## 2026-09-11T01:40:13Z — centerColumn_white_run_le_period (haiku, proved)

**Proof shape**: Strong induction on position offset k. For k < p, use hrun directly. For k ≥ p, apply periodicity (hc) to shift back to position k - p, then recurse via the induction hypothesis. This proves all cells from t onward are white. Then cite centerColumn_not_eventually_constant to get a contradiction.

**Key patterns used**:
- Strong induction via `Nat.strong_induction_on` with direct lambda-style proof
- Direct application of periodicity lemma hc with `by omega` bounds
- norm_num to close the final Bool contradiction (false = true)
- Import of CenterColumnNotEventuallyConstant to access the contradiction lemma

**Imports needed**: Rule30.Proofs.CenterColumnNotEventuallyConstant (in addition to Rule30.Basic)

**Axioms**: propext, Classical.choice, Quot.sound (standard for P1 nodes)

## 2026-09-12T17:17:52Z — rightDiagonal_cone (sonnet, proved)

rightDiagonal_cone (sonnet, proved) — pure retrieval, cheap (~$1 total, well under budget).

The brief named `explorer/sextant10_scratch_cone.lean` as Sextant's route, and it was there: `cone_rightDiagonal` at lines 88-96, exactly the statement needed, already proved. The scratch file is 355 lines with a lot more in it (edge_gap machinery for a follow-on node about the exact gap size, a backward-line telescoping identity with `decide`-checked examples) — none of that is this node's job. Traced the actual dependency chain of `cone_rightDiagonal` by hand: it cites `cone_row_pow`, which cites `cone_row` and `rightDiagonal_periodicFrom_pow_le`, which cites `rightDiagonal_periodicFrom_pow` and `periodicFrom_mul` (both already-closed nodes) and `rightDiagonal_first_failure` (also closed). Nothing else in the scratch file's nine imports was needed — dropped `EvolveLeftEdge`, `MinimalPeriodDvd`, `RightDiagonalAntiperiodicOfOddDriver`, `RightDiagonalPeriodicFromStepOfEvenDriver`, `RightDiagonalPeriodicFromStep` entirely, along with every theorem past `cone_row_pow` (the `edge_gap`/`edge_gap_eq` family, `antiperiodic_odd_mul`, the backward-line section). Built clean first try with the pruned four-theorem file (three `private` helpers + the public theorem), confirming the traced dependency set was complete.

This is now a well-worn pattern across this whole project's notebooks (see many entries under leftDiagonal_period_unbounded, column_settledConfig_eq, etc.): a scratch file from a theorist/connector session is a whole exploration, and a proof file is the one path through it that this specific node needs — prune aggressively rather than importing the theorist's whole toolkit.

The math itself (for the record, since it's not mine): the cone condition in the right-diagonal tower's own coordinates — row 2^k is white at the k positions just left of its black right edge — reduces to "no black cell exists in that range" via a least-element (Nat.find) argument: if some cell in [2^k - k, 2^k - 1] were black, take the *nearest* one to the edge (index m), and rightDiagonal_first_failure pins down that a disagreement between diagonal m and its own value one full period (2^k) earlier starts exactly at that nearest black cell — but diagonal m is provably periodic with period 2^k from the origin (since m ≤ k), a contradiction. This retires crystal 11's "second ingredient", which had been mis-priced as needing Rowland's own induction rather than three already-closed nodes plus this least-element argument.

Only warning: `push_neg` deprecation (superseded by `push Not` in this Mathlib snapshot) — harmless, build succeeds, consistent with other notebooks' notes that this deprecation is cosmetic here.

Band, per CLAUDE.md's reporting contract: **Known as a statement, project-internal as a proof** — exactly as the brief's own Band field says. This is Rowland 2006 Theorem 1 for rule 30 (his section 1, lines 123-126, mirror orientation), a real fact about rule 30 already in the literature. The proof itself is project-internal: it retires an unseeded ingredient crystal 11 had mis-priced, and supplies `rightDiagonal_edge_gap_eq`-style follow-on work with a base case, but the brief is explicit that no successor reaches Prize 1 — the region was fenced the same day from both the recurrence side (this) and the self-similar-group side (Portage).
...
- The harness verifies with a generated check theorem — `theorem harness_check : type_of% Statements.rightDiagonal_edge_gap_eq := rightDiagonal_edge_gap_eq` — against the captain's statement. So your theorem must be named exactly `rightDiagonal_edge_gap_eq` and have exactly the stated type. A weakened or generalized restatement fails, even one you could prove.
...
- `rule30_alternating_step` in `Rule30.Proofs.Rule30AlternatingStep` — An alternating block left of the origin shrinks by one per step: if the cells at 0, -1, ..., -(L+1) alternate black-white starting black, then after one step the cells at 0, ..., -L do. The engine of the black-run correspondence. DOES NOT PROVE: unconditional and true of every configuration, one unfolding of the local rule under an alternating hypothesis. Proposed by Seeder 2026-09-10; the route failed with `unknown tactic` and the cause is unexplained -- see the board row seed-check-reports-unknown-tactic-and-i-could-not-find-the-cause.
- `column_alternating_of_black_run` in `Rule30.Proofs.ColumnAlternatingOfBlackRun` — A LONG BLACK RUN FORCES A DEEP ALTERNATING BLOCK: if the centre column is black at t, ..., t+L then the cells at 0, ..., -L alternate at time t. With its converse this makes BLACK RUNS AND ALTERNATING DEPTH THE SAME MEASUREMENT read in two directions -- one along time at the origin, one across space to the left. DOES NOT PROVE: unconditional, and it moves a question rather than answering one; whether either object occurs at arbitrary size is exactly what is open. Proposed by Seeder 2026-09-10; the route failed with `unknown tactic` and the cause is unexplained -- see the board row seed-check-reports-unknown-tactic-and-i-could-not-find-the-cause.
- `centerColumn_not_isEventuallyPeriodic_of_long_black_runs` in `Rule30.Proofs.CenterColumnNotIsEventuallyPeriodicOfLongBlackRuns` — ARBITRARILY LONG BLACK RUNS WOULD PROVE PRIZE 1: if the centre column contains black runs of every length, arbitrarily late, it is not eventually periodic. DOES NOT PROVE, and the honest band matters here: THE IMPLICATION IS TRIVIAL AND TRUE OF ANY BOOL SEQUENCE -- an eventually periodic sequence has runs bounded by its period, so unbounded runs refute periodicity in one line, and a mathematician would not call this a result. What it buys is a RESTATEMENT of P1 as a question about run lengths, which is measurable where P1 is not: the longest run seen so far is 16 identical cells at M = 22711. Whether the runs are unbounded is open and is NOT known to be easier than P1. Proposed by Seeder 2026-09-10; ROUTE VERIFIED, seed check reported closes the statement.
- `column_black_run_of_alternating` in `Rule30.Proofs.ColumnBlackRunOfAlternating` — A deep alternating block makes a long black run in the centre column: if the cells at 0, ..., -L alternate at time t, the centre column is black at t, t+1, ..., t+L. One half of the correspondence. DOES NOT PROVE: unconditional -- it says a deep alternating block FORCES a long run, not that either occurs. Proposed by Seeder 2026-09-10. ROUTE NOT CHECKABLE: it imports the proof file of a sibling in this same tier, which does not exist until the tier lands -- see the board row a-route-that-imports-a-sibling-proposal-can-never-be-checked. The route may be good and there is no way to find out from here.
- `column_alternating_shrink` in `Rule30.Proofs.ColumnAlternatingShrink` — A maximal alternating block shrinks by exactly one and stays maximal: if the block alternates to depth L and fails at L+1, then at the next step it alternates to depth L-1 and fails at L. The sharp form -- the block does not merely shrink, it keeps its edge. DOES NOT PROVE: unconditional, and nothing about how such blocks arise. Proposed by Seeder 2026-09-10. ROUTE NOT CHECKABLE: it imports the proof file of a sibling in this same tier, which does not exist until the tier lands -- see the board row a-route-that-imports-a-sibling-proposal-can-never-be-checked. The route may be good and there is no way to find out from here.
- `centerColumn_not_isEventuallyPeriodic_of_deep_alternating` in `Rule30.Proofs.CenterColumnNotIsEventuallyPeriodicOfDeepAlternating` — ARBITRARILY DEEP ALTERNATING BLOCKS WOULD PROVE PRIZE 1: the same sufficient condition read across SPACE instead of along time, via the black-run correspondence. DOES NOT PROVE: it inherits the triviality of the implication it composes with, and it moves P1 to a question about a spatial pattern with no evidence that the spatial question is easier. Its value is that the alternating block is a LOCAL, CHECKABLE object where 'the centre column is aperiodic' is not. Proposed by Seeder 2026-09-10. ROUTE NOT CHECKABLE: it imports the proof file of a sibling in this same tier, which does not exist until the tier lands -- see the board row a-route-that-imports-a-sibling-proposal-can-never-be-checked. The route may be good and there is no way to find out from here.
- `centerColumn_white_run_le_period` in `Rule30.Proofs.CenterColumnWhiteRunLePeriod` — A PERIODIC CENTRE COLUMN HAS SHORT WHITE RUNS: if the centre column has period p > 0 from N, no maximal white run beginning at or after N reaches length p. Two lines from centerColumn_not_eventually_constant, proved 2026-09-10. WHY THIS NODE EXISTS, AND IT IS A CORRECTION RATHER THAN AN ADDITION: the switch-index reformulation was landed and reported by the captain as applying to the family X_b and NOT to the seed, because rule 30's centre column has long runs and nothing bounds them. That is true unconditionally and IRRELEVANT WHERE THE RESIDUAL IS STATED -- the residual's hypothesis IS eventual periodicity, and under it the runs are bounded, so the finite alphabet is available on the seed exactly where it is needed. DOES NOT PROVE: the hypothesis is the negation of P1, so it is counterfactual and yields no measurement; it bounds the alphabet of an object nobody can construct. Proposed by Groma (connector) 2026-09-10.
- `centerColumn_black_run_lt_start` in `Rule30.Proofs.CenterColumnBlackRunLtStart` — A BLACK RUN OF THE CENTRE COLUMN IS SHORTER THAN THE TIME IT STARTS AT: black at every time from a to a+L with a >= 1 forces L < a. While the centre stays black the cells just left are forced to alternate one cell deeper per step (column_alternating_of_black_run), and the cone will not allow a deep checkerboard -- at row a the two leftmost cells that exist, at -a and -(a-1), are both black. So the forced block cannot reach depth a. a >= 1 is not a technicality: at a = 0 the cone's left edge and the origin are the same cell. Measured 0 violations over 1,000,344 maximal runs to N = 2,000,000, tight at a = 3. ORIENTATION-SENSITIVE, which this project has twice been burned for lacking: rule 86, rule 30 mirrored, has the same centre-column runs and fails the underlying alternation at 316 of 968 cells. DOES NOT PROVE: nothing towards Prize 2. It bounds one run and says nothing about how often runs occur or about any density. Proposed by Seeder (seeder, 2026-09-11) for region P2; landed by the captain after review.
- `centerColumn_white_run_lt_start` in `Rule30.Proofs.CenterColumnWhiteRunLtStart` — THE WHITE TWIN, AND IT IS NOT SHARP: white at every time from a to a+L with a >= 1 forces L < 3a. Splits on the colour of the column one cell left of the origin, which is monotone inside a white run (white_run_forbidden). Before it turns black, reading rule 30 backwards (sideways_inverse) forces an all-white triangle the black left edge stops; after, it forces the checkerboard in the OPPOSITE phase to a black run's, which the cone's two adjacent black cells stop. The two compose to L < 3a. DOES NOT PROVE: not the sharp bound, and nothing here explains the gap -- measured, a maximal white run beginning at a has length at most a/2, where this proves only < 3a. 3 is stated because 3 is what this argument gives. Nothing towards Prize 2. Proposed by Seeder (seeder, 2026-09-11) for region P2; landed by the captain after review.
- `centerColumn_window_not_constant` in `Rule30.Proofs.CenterColumnWindowNotConstant` — THE INTERFACE NODE: from time a to time 4a the centre column is neither all black nor all white, for every a >= 1. All black contradicts the black bound at L = a; all white contradicts the white bound at L = 3a. Stated with the same 3a on both sides so the counting node has one window rather than two. This is the form in which rule 30 says something about the centre column THAT A Bool SEQUENCE CAN FAIL, and the counting node is the only thing that consumes it. DOES NOT PROVE: nothing towards Prize 2; it is the two run bounds restated. The window is far too wide -- measured, every window [a, 2a] already holds both colours -- and the width is the white bound's constant rather than a fact about the picture. Proposed by Seeder (seeder, 2026-09-11) for region P2; landed by the captain after review.
- `centerColumnCount_ge_of_pow` in `Rule30.Proofs.CenterColumnCountGeOfPow` — BELOW 5^n THE CENTRE COLUMN HAS AT LEAST n CELLS OF EACH COLOUR. The windows [5^k, 5^(k+1)) for k < n are disjoint and cover [1, 5^n), and each contains the window [5^k, 4*5^k] that centerColumn_window_not_constant says holds both colours -- 4*5^k < 5^(k+1) is the whole reason the base is 5 rather than 4. The derivation was checked in Lean with the window statement taken as a hypothesis rather than assumed available, so what is asked for is what its consumer needs. It gives |2*count N - N| <= N - 2*floor(log_5 N), landing in the residual's own vocabulary and the same inequality that centerColumn_density_tendsto_half_iff_excess_nat states the prize in. DOES NOT PROVE: nothing towards Prize 2, and the gap is not a matter of sharpening. N - 2 log_5 N is N(1-o(1)) where the prize needs o(N), and Talus's attack of 2026-09-10 caps the whole run route at N(1 - 1/(c log N)) for ANY run bound however sharp, so NO SUCCESSOR OF THESE FOUR NODES REACHES IT. Its value is as a fence plus one non-vacuous count statement. CAPTAIN'S CORRECTION TO THE PROPOSAL: the seeder wrote that all twelve closed P2 nodes hold of every Bool sequence. Ten do. centerColumn_zero and centerColumnCount_eq_stepMod_count are rule-30 statements, but neither constrains the density -- one fixes a single value, the other re-encodes the count without yielding an inequality. The refined claim this tier rests on is that this would be the first P2 node giving a rule-30-dependent BOUND on the counts. Proposed by Seeder (seeder, 2026-09-11) for region P2; landed by the captain after review.
- `rightDiagonal_cone` in `Rule30.Proofs.RightDiagonalCone` — The cone condition in the tower's own coordinates: row 2^k is white at the k positions immediately left of its black right edge. Rowland 2006 Theorem 1 for rule 30, his section 1 lines 123-126 in mirror orientation. Crystal 11 carries this as its unseeded second ingredient and prices it as 'Rowland's own induction, not a finite check' -- THAT PRICING IS WRONG and correcting it is why this node exists. Sextant proved it 2026-09-12 as a least-element argument over three closed nodes, the last of which (rightDiagonal_first_failure) landed the day AFTER crystal 11 was written, which is why nobody saw the route. Route in hand: explorer/sextant10_scratch_cone.lean, theorem cone_rightDiagonal, accepted by lake env lean with axioms inside the allowlist. DOES NOT PROVE: nothing towards any prize, and the region was fenced the same afternoon -- Sextant closed the right edge for P1 from the recurrence side and Portage from the self-similar-group side, both 2026-09-12, so no successor of this node reaches Prize 1. Seeded as a ROUTE CLOSURE: it retires crystal 11's second ingredient and supplies rightDiagonal_edge_gap_eq. Band: known as a statement, project-internal as a proof. [2026-09-12, Rowan, from Sextant C2]
