...
Topic: falsify rightDiagonal_period_doubles_iff_odd_weight: the MINIMAL period of right diagonal k is 2L when g_k has odd weight over one period L and L otherwise; measured to 140,000 terms with every depth covered by at least 2 guaranteed periods; the right diagonals have no transients, so this is a doubling criterion on the side where index 0 is not inside one

Attack document: c:/Users/dibuj/dev/rule30/docs/attacks/2026-09-08-falsify-rightdiagonal-period-doubles-iff-odd-weight-the-minimal-period-of-right-diagonal-k-is-2l-when-g-k-has-odd-weight.md
...
84 theorems on the board: 79 proved, 4 open, 1 claimed, 0 blocked, 0 abandoned. 0 of 84 without an object.
...
**Cited by.** column_settledConfig_eq, leftDiagonal_mul_pow_eq_settledCenter
**Status.** proved, size M
...
**What this says.** The front's law at each diagonal's own period.
**Hypotheses.** (seeded statement; not yet checked)
- `(k j : ℕ)`
...
**Cited by.** column_settledConfig_eq, leftDiagonal_periodicFrom_pow, rightDiagonal_periodicFrom_pow
**Status.** proved, size S
