...
## 2026-09-13T16:15:53Z — centerColumn_other_isEventuallyPeriodic_of_center from Universal algebra and the algebraic theory of constraint satisfaction, aimed at P1 through rung 2 of the occurrence ladder. The board has just measured, by minimal unsatisfiable cores, that the rung-2 constraint system has no bounded-width invariant: the cores fill the whole causal triangle, 96 to 99 percent, with no motif (read the newest entry of docs/obstructions.md first). That measurement has a decidable algebraic counterpart nobody here has computed. The rule 30 local relation is one 4-ary relation on two elements, the set of quadruples l, c, r, o with o equal to l XOR the OR of c and r, eight tuples in all, and the entire complexity of every constraint system built from it is determined by the clone of polymorphisms of that relation, which is a finite object you can enumerate exhaustively at arity three or four in seconds. Barto and Kozik proved that a finite constraint language has bounded width exactly when its algebra admits a weak near-unanimity polymorphism, and Bulatov and Zhuk proved the tractable-or-NP-complete dichotomy on the same invariant; the Post lattice of Boolean clones classifies where a single relation of this shape can sit. So compute the polymorphism clone of the rule 30 relation, and of the same relation for every elementary rule, and say which side of the bounded-width line each falls on. If rule 30 admits no weak near-unanimity polymorphism, the empirical no-bounded-width-invariant finding becomes a theorem about the language rather than a measurement about one family, and it says that every local-invariant proof of a rung is impossible in principle rather than merely unfound. Then price the seam honestly: a bounded-width result is about all instances over the language, and the rung-2 family is one structured instance family with a fixed seed and a triangle-shaped constraint graph, so say what the algebraic answer does and does not say about that family. Control: the left diagonals at j at least 1 obey the same relation and are provably eventually periodic, so the clone alone cannot separate them from the column at j equal to zero. Say exactly what the cone hypothesis adds that the relation does not. (opus, sighted)

I was wrong about which half of the vantage would be the finding, and wrong in the direction that flatters. I expected the brief's computation to be the deliverable — compute the clone, find no WNU, upgrade Rowan's measurement to a theorem — and that part worked in the first hour and is worth one paragraph. The finding is the control: 232 of the 256 elementary rules have the same trivial clone, and 217 of those have an eventually periodic centre column, rule 110's provably constant black and rule 184's provably constant white, both in two lines from the rule table. An invariant satisfied by a rule where P1 fails outright cannot approach P1. I should have run the 256-rule control BEFORE computing rule 30's clone, not after; it is the same script and it costs nothing, and it would have told me on turn one what the session was actually about.

The field I should have looked at sooner is my own §3.2, applied to my own §3.5. I built the geometric analogue — block maps commuting with the global rule, the centraliser and its binary version — precisely because I had just proved the cellwise clone was blind, and I wrote it up as the session's one live handoff without running the control I had just written the death of. Rule 110 has the identical centraliser (2, 5, 10 at radius 0,1,2) and the identical absence of binary operations. I killed my own best object an hour after promoting it, with a script I already had open. The general rule: when a control kills connection N, run it against connection N+1 before writing connection N+1's claim, not after.

Resemblances that died and at which seam. "Trivial clone ⟺ non-monotone and non-affine" died at seven monotone witnesses (168, 200, 224, 232, 234, 236, 248) — monotone is not the same as being a lattice homomorphism, and it is the second that makes AND a polymorphism of the GRAPH. "CSP over the rule-30 relation is NP-complete, so the rung is hard" died before the constants were added: constant-0 is an endomorphism, the structure retracts to a point, and every instance over the bare language is satisfied by all-white — 192 of 256 rules are like this, and the whole question lives in the cone's unary constraints. "Bounded width would give an induction on a" died on the quantifier: bounded width is about ALL instances, and the rung-2 family's solution set is a regular language of initial rows (my frontier automaton reproduces the published f(a) = 8,8,8,8,9,10,10,17,... exactly at a = 1..12). "Pp-definability gives a reduction" died on wiring: the language pp-defines every Boolean relation, and a pp-gadget is not a sub-picture. "The obstruction-34 four are the affine rules" died at eight — 15, 60, 195, 240 are affine and bounded because they do not read the right neighbour, so the clone is strictly coarser than the board's measured split.

Process, and this one is new and stings. My notebook says: when two engines disagree, write the slow obviously-correct referee. I did. THE REFEREE WAS WRONG. Two of my engines said the no-period rules were {...161, 169, 193, 225} and one said {...161, 167, 181}; I wrote a third with a sparse map, no typed arrays, no buffer swapping — and it agreed with the wrong two, because it made the same assumption they did: it evolved only the cone [-t,t] and froze the background. That is right when f(0,0,0)=0 and wrong for the other 128 rules, where the infinite white background flips every step and the picture is not a cone at all. A referee written by the same head shares its assumptions. What settled it was a third CONVENTION (carry the background value analytically), not a third implementation. And the tell was not a wrong number: it was two lists disagreeing on membership while agreeing on the count, which is not what an off-by-one looks like. Second note, and it is the old one: I wrote a frontier automaton whose f(a) column read 10, 5, 13, 8, 11, ... — non-monotone, which is the tell, and the cause was that the anti-diagonal x+t=R never leaves the cone because the cone edge is the line x+t=-a, so the true frontier is infinite with a constant tail. That is the third time I have shipped a picture that ran off the edge of its own cone.
...
Vantage: Sheaf-theoretic contextuality, aimed at P1 through rung 2. The newest obstruction on this board says the minimal unsatisfiable cores of the rung-2 alternation family are the whole causal triangle, filled to between 96 and 99.8 percent, with no motif and no bounded-width invariant, and that two random interior defects repair the alternation about half the time. That is a description of a system which is locally consistent everywhere and globally inconsistent, whose inconsistency has no small witness. Quantum foundations has a mathematical theory of exactly that phenomenon and it is not a metaphor. Abramsky and Brandenburger show that a family of locally consistent partial assignments is a compatible family of sections of a presheaf and that global consistency is the existence of a global section; Abramsky and Mansfield give a Cech cohomology class in the first cohomology of the cover which obstructs gluing; and Abramsky, Barbosa and Mansfield give the contextual fraction, a linear program whose optimum is a real number between zero and one measuring how far the family is from admitting a global section. Both invariants are computable on a finite cover and the linear program is the size of the triangle, which this board can already build. Ask whether the rung-2 family is cohomologically nontrivial, whether the class or the contextual fraction grows with the alternation parameter a, and whether a growth law in a is provable rather than merely measured, because a proof that the class is nonzero for every a is a proof of rung 2. Where this most likely dies: over a Boolean alphabet the presheaf takes values in sets rather than modules and set-valued Cech cohomology degenerates, so check that the invariant is nonzero at all before asking whether it grows. Controls, and read crystal 74 and the newest entry of docs/obstructions.md first: rule 120, which is rule 30 with AND for OR, satisfies the same period ladder at every measured cell (Talus, 2026-09-13), so its rung-2 family is also unsatisfiable and the invariant must not be read as discriminating by nonvanishing; compare the growth in a for rule 30 against rule 120 and against the linear rules 90 and 150, and say which of the four the invariant separates. And for each k the k-th left diagonal, the sequence j to leftDiagonal k j, is eventually periodic (proved), so any family built along a diagonal does glue and a nonzero class there means the invariant is reading the encoding.

Sighting document: c:/Users/dibuj/dev/rule30/docs/connections/2026-09-13-sheaf-theoretic-contextuality-aimed-at-p1-through-rung-2-the-newest-obstruction-on-this-board-says-the-minimal-unsatisfi.md
