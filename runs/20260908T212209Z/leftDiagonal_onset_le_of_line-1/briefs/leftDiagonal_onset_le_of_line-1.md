...
- The harness verifies with a generated check theorem — `theorem harness_check : type_of% Statements.leftDiagonal_onset_le_of_line := leftDiagonal_onset_le_of_line` — against the captain's statement. So your theorem must be named exactly `leftDiagonal_onset_le_of_line` and have exactly the stated type. A weakened or generalized restatement fails, even one you could prove.
